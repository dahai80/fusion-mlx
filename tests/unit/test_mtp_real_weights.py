...
~15 s wall on warm cache). Gated on ``FUSION_MLX_RUN_HEAVY_TESTS=1``
...
run::
    ...
    FUSION_MLX_RUN_HEAVY_TESTS=1 pytest tests/test_mtp_real_weights.py -xvs
"""
...
# This whole file is gated on FUSION_MLX_RUN_HEAVY_TESTS=1. The unit
# tests in ``test_mtp_spec_decode.py`` continue to cover the
# control-flow surfaces in the normal CI sweep.
_HEAVY = os.environ.get("FUSION_MLX_RUN_HEAVY_TESTS") == "1"

pytestmark = pytest.mark.skipif(
    not _HEAVY,
    reason=(
        "Heavy real-weights probe (5 GB base + 131 MB sidecar). "
        "Set FUSION_MLX_RUN_HEAVY_TESTS=1 to run."
    ),
)
