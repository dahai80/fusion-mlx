...
class TestResolveBackend:
    ...
    def test_legacy_ltx_autodetect(self):
        # Legacy LTX-Video (0.9.x) has no MLX port -> stub. Detected by name
        # substring and by HF repo id.
    ...
    def test_backends_registry_has_all(self):
        # ltx2 + wan2 ship real mlx-video impls; ltx_video_legacy + cogvideo
        # are stubs that raise NotImplementedError (no MLX port exists upstream).
        assert set(BACKENDS) == {"ltx2", "wan2", "ltx_video_legacy", "cogvideo"}


class TestUnimplementedBackends:
    # Legacy LTX-Video and CogVideoX have no MLX port and are not shipped by
    # mlx-video. The stubs must (a) validate permissively so requests reach the
    # backend, then (b) raise NotImplementedError pointing at the upstream
    # issue tracker and naming a real alternative.

    @pytest.mark.parametrize("model", ["ltx-video", "cogvideo"])
