...
class TestFlashKDAReference:
    ...
    def test_initial_state_propagated(self):
        ...
        out_with, state_with = fwd_ref(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias, initial_state=init_state)
        out_without, state_without = fwd_ref(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias)
        mx.eval(out_with, state_with, out_without, state_without)
        assert not mx.allclose(out_with, out_without)

    def test_deterministic(self):
        q, k, v, g, beta, A_log, dt_bias = _make_inputs()
        out1, state1 = fwd_ref(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias)
        out2, state2 = fwd_ref(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias)
...
class TestFlashKDABridge:
    def test_bridge_returns_same_as_reference(self):
        ...
        out_bridge, state_bridge = fwd(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias)
        out_ref, state_ref = fwd_ref(q, k, v, g, beta, scale=1.0, A_log=A_log, dt_bias=dt_bias)
