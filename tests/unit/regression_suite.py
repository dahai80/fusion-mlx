...
"""Comprehensive regression and edge case test suite for fusion-mlx.

Standalone script — the doctor harness invokes it via subprocess
against a live server (``[py, str(script)]`` in
``fusion_mlx/doctor/checks/api.py``). NOT meant for pytest collection;
...
_PORT = os.environ.get("FUSION_MLX_PORT", "8777")
...
if __name__ == "__main__":
    ...
    for i, test_fn in enumerate(
        ...
        try:
            ...
            logger.exception("Test %d raised exception", i)
            results[i] = False
        print()
