...
"""Tests for the `fusion-mlx upgrade --dry-run` flag.
...
from fusion_mlx import _version_check as vc
from fusion_mlx.cli import upgrade_command
...
def _stub_brew_with_upgrade_available(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: vc.InstallInfo(
            method="brew",
            binary_path="/opt/homebrew/bin/fusion-mlx",
...
def test_dry_run_does_not_invoke_subprocess(monkeypatch, capsys):
    ...
    assert "Current:  fusion-mlx 0.9.3" in out
    assert "Latest:   fusion-mlx 0.9.4" in out
    assert "brew upgrade dahai80/homebrew-fusion-mlx/fusion-mlx" in out
