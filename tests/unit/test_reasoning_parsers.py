...
from fusion_mlx.reasoning.base import DeltaMessage, ReasoningParser
...
from fusion_mlx.reasoning.gpt_oss_parser import (
    _CHANNEL_RE,
    _STRUCTURAL_TOKENS,
...
class TestBaseThinkExtractReasoning:
    ...
    def test_multiple_think_tags_uses_first_partition_only(self):
        ...
        # matches them (see ``fusion_mlx/api/utils.py::THINK_PATTERN``).
...
class TestResidualThinkTagSweep:
    ...
    @pytest.mark.skip(reason="sanitize_output not available in fusion_mlx.api.utils")
    def test_orphan_closer_left_for_downstream(self):
        from fusion_mlx.api.utils import sanitize_output
        from fusion_mlx.reasoning.deepseek_r1_parser import (
            DeepSeekR1ReasoningParser,
        )
