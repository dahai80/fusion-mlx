...
def test_load_model_alias_resolver_handles_every_import_shape():
    ...
    Shapes covered:
      1. ``from fusion_mlx.server import load_model`` → ``load_model(...)``
      2. ``from fusion_mlx.server import load_model as lm`` → ``lm(...)``
    ...
    """
    shapes = {
        "direct": "from fusion_mlx.server import load_model\nload_model('q')\n",
    ...
    for path in pkg_root.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            if (absolute_server or relative_server) and any(
                ...
                offenders.append(
                    f"{rel}:{node.lineno} uses `from fusion_mlx.server "
...
# showed `from fusion_mlx.server import load_model as lm` and
# `from fusion_mlx import server; server.load_model(...)` both slip a
...
def _discover_entrypoints() -> set[str]:
    ...
    for path in root.rglob("*.py"):
        ...
        # call scan, so a new entrypoint using `from fusion_mlx import
        # server` / `import fusion_mlx.server as srv` doesn't slip
        # discovery.
...
def test_registry_invariants():
    ...
    or duplicated to defeat the derived gates:
      ...
      - Hygiene: every ``model_config_field`` must actually exist on
        ...
        execution time.
    """
    from fusion_mlx.model_auto_config import ModelConfig
...
def test_routing_override_kwargs_are_forwarded_to_load_model():
    ...
    def _find_load_model_calls(source: str) -> list[ast.Call]:
        ...
        # (`from fusion_mlx.server import load_model as lm`,
        # `import fusion_mlx.server as srv`, etc.). The previous literal-
...
def _make_engine_core_for_override_test(monkeypatch, cfg, *, base=None):
    ...
    try:
        ...
        from fusion_mlx import model_auto_config as mac
        from fusion_mlx.model_auto_config import ModelConfig
    except RuntimeError as exc:
        pytest.skip(
