...
* Post-R12-4, ``strict=true`` runs the engine UNCONSTRAINED, validates
  ...
  also fails. The disable flag ``FUSION_MLX_STRICT_JSON_SCHEMA=off``
...
pytestmark = pytest.mark.xfail(
    reason="strict=False: strict json_schema constraint matrix not ported to api/ routes yet",
    strict=False,
)
...
def test_strict_constraint_matrix_disable_flag_returns_200_for_all(monkeypatch):
    """With ``FUSION_MLX_STRICT_JSON_SCHEMA=off`` every row of the
