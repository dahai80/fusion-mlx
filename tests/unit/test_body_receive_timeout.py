...
``FUSION_MLX_BODY_RECEIVE_TIMEOUT_SECONDS``, 0 disables), the
...
def test_timeout_disabled_when_zero():
    """``FUSION_MLX_BODY_RECEIVE_TIMEOUT_SECONDS=0`` (the documented
...
def test_h14_env_var_override_reduces_timeout(monkeypatch):
    ...
    ``FUSION_MLX_BODY_RECEIVE_TIMEOUT_SECONDS`` reduces the per-chunk
...
def test_h14_default_timeout_value_is_15_seconds():
    ...
    15 s default being applied when ``FUSION_MLX_BODY_RECEIVE_TIMEOUT_SECONDS``
