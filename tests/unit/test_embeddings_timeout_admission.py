...
class TestEmbeddingInputFourShapes:
    ...
    def test_str_accepted(self):
        from fusion_mlx.api.models import EmbeddingRequest
...
@pytest.mark.xfail(
    strict=False,
    reason=(
        "strict=False: H6 pre-tokenized embeddings input not ported after the "
        "engine refactor (api/embeddings_routes uses EmbeddingRequest "
        "str|list[str] only; EmbeddingEngine has no embed_tokens)."
    ),
)
...
class TestEmbeddingEngineEmbedTokens:
    ...
    def test_embed_tokens_method_exists(self):
        from fusion_mlx.embedding import EmbeddingEngine
...
class TestAdmissionControl:
    ...
    def test_add_request_raises_backpressure_at_cap(self):
        ...
        from fusion_mlx.scheduler import BackpressureError, Scheduler, SchedulerConfig
    ...
    def test_check_admission_reservation_is_atomic(self):
        ...
        from fusion_mlx.scheduler import BackpressureError, SchedulerConfig
