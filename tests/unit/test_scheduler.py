...
import fusion_mlx.scheduler as scheduler_module
from fusion_mlx.request import Request, RequestOutput, RequestStatus, SamplingParams
...
class _ParserStopSession:
    def process_token(self, token_id):
        from fusion_mlx.parsers.output_parser import OutputParserTokenResult
    ...
    def finalize(self):
        from fusion_mlx.parsers.output_parser import OutputParserFinalizeResult
...
class TestPrefillAbortInterrupt:
    ...
    def test_external_prefill_abort_reclaims_metal_before_raise(
        ...
        from fusion_mlx.scheduler import _PrefillAbortedError
...
class TestSyncAndClearCache:
    ...
    def test_swallows_generation_stream_thread_error(self):
        ...
        """
        from fusion_mlx import scheduler as sched_mod
...
class TestSchedulerBoundarySnapshots:
    ...
    def test_cleanup_finished_pre_evals_intermediate_boundary_snapshots(
        ...
        with (
            ...
            patch("fusion_mlx.scheduler.sched_response._safe_sync_stream"),
        ):
    ...
    def test_boundary_snapshot_synchronizes_generation_stream(
        ...
        with patch("fusion_mlx.scheduler.sched_boundary.mx") as mock_mx:
    ...
    def test_cleanup_finished_synchronizes_before_cache_store(
        ...
        with patch("fusion_mlx.scheduler.sched_response.mx") as mock_mx:
...
class TestSchedulerRotatingBlockAlignment:
    ...
    def test_cleanup_finished_extends_deferred_clear_for_concurrent_completions(
        ...
        with patch("fusion_mlx.scheduler.sched_response.mx"):
...
class TestSchedulerSSDLayerSignature:
    ...
    def test_refresh_uses_final_turboquant_layout_and_sweeps(
        ...
        from fusion_mlx.cache.paged_ssd_cache import PagedSSDBlockMetadata
...
class TestGenerationOverflowRecovery:
    ...
    def test_generation_overflow_reschedules_for_serial_retry(
        ...
        with patch("fusion_mlx.scheduler.sched_step._sync_and_clear_cache"):
            output = scheduler.step()
...
class TestStoreCacheAdmissionBackpressure:
    ...
    def test_drain_pending_async_removes_releases_done_entries_out_of_order(
        ...
        with patch("fusion_mlx.scheduler.sched_cache._safe_sync_stream"):
...
class MockTokenizer:
    """Real tokenizer stub for think-prefix detection tests.

    Plain class (not MagicMock) so tests can set instance attributes
    (think_start_id/think_end_id) and install class-level PropertyMock
    overrides (test_think_start_id_raises_type_error). Think-token
    attributes are absent by default, so ``_get_think_token_id``'s
    ``getattr(..., None)`` returns None - matching real tokenizers
    lacking thinking support. eos_token_id=2 mirrors the contract
    referenced by other scheduler tests. encode returns a list so
    ``_get_xtc_special_tokens``'s ``encode("\\n").extend(...)`` works
    during Scheduler.__init__.
    """
