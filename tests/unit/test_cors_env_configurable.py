...
(``FUSION_MLX_CORS_ALLOW_ORIGINS`` / ``_METHODS`` / ``_HEADERS`` /
...
def fresh_app(monkeypatch: pytest.MonkeyPatch) -> Iterator[FastAPI]:
    ...
    Each test also gets a clean env (no leaked ``FUSION_MLX_CORS_*`` from
    other tests).
    """
...
# multi-tenant deployments lock down via FUSION_MLX_CORS_ALLOW_ORIGINS.
# ──────────────────────────────────────────────────────────────────────
...
def test_env_explicit_origin_matching(
    ...
    """``FUSION_MLX_CORS_ALLOW_ORIGINS=https://chat.openai.com`` → matching
...
def test_default_methods_do_not_include_destructive_verbs(
    ...
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    _server_mod().configure_cors_from_env(cli_origins=None)
...
def test_env_methods_override(
    ...
    """``FUSION_MLX_CORS_ALLOW_METHODS=POST,OPTIONS`` narrows the default
    allowlist further. The operator can lock down to POST + OPTIONS for
    a webhook-style deployment."""
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_METHODS", "POST,OPTIONS")
    _server_mod().configure_cors_from_env(cli_origins=None)
...
def test_wildcard_logs_warning_and_works(
    ...
    with caplog.at_level("WARNING", logger="vllm_mlx.server"):
        origins = _server_mod().configure_cors_from_env(cli_origins=None)
    assert origins == ["*"]
    # No wildcard warning under #641 — explicit ``*`` is by-design.
    assert not any(
        "wildcard" in rec.message.lower() for rec in caplog.records
    ), f"Explicit '*' should not warn under #641; got {[r.message for r in caplog.records]!r}"
...
def test_cli_origins_override_env(
    ...
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://from-env.example")
...
def test_malformed_max_age_falls_back_to_default(
    ...
    """Bad ``FUSION_MLX_CORS_MAX_AGE`` logs a warning and uses 3600 s. We
    don't crash startup on a typo — same shape as the ``--max-request-bytes``
    fallback added in PR #732."""
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    monkeypatch.setenv("FUSION_MLX_CORS_MAX_AGE", "not-a-number")
    with caplog.at_level("WARNING", logger="vllm_mlx.server"):
        _server_mod().configure_cors_from_env(cli_origins=None)
    assert any(
        "FUSION_MLX_CORS_MAX_AGE" in rec.message for rec in caplog.records
...
def test_empty_csv_origin_value_fails_closed_with_warning(
    ...
    assert origins == []
    assert any(
        "FUSION_MLX_CORS_ALLOW_ORIGINS" in rec.message
        and "fail-closed" in rec.message.lower()
        for rec in caplog.records
    ), f"Expected a fail-closed WARNING naming FUSION_MLX_CORS_ALLOW_ORIGINS; got {[r.message for r in caplog.records]!r}"
...
def test_empty_methods_env_warns_and_falls_back(
    ...
    """An operator typo like ``FUSION_MLX_CORS_ALLOW_METHODS=" , "`` must
    ...
    """
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_METHODS", " , ,, ")
    with caplog.at_level("WARNING", logger="vllm_mlx.server"):
        _server_mod().configure_cors_from_env(cli_origins=None)
    assert any(
        "FUSION_MLX_CORS_ALLOW_METHODS" in rec.message
...
def test_empty_headers_env_warns_and_falls_back(
    ...
    """Same as the methods case — ``FUSION_MLX_CORS_ALLOW_HEADERS=" , "``
    ...
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_HEADERS", " , ,, ")
    with caplog.at_level("WARNING", logger="vllm_mlx.server"):
        _server_mod().configure_cors_from_env(cli_origins=None)
    assert any(
        "FUSION_MLX_CORS_ALLOW_HEADERS" in rec.message
        and "empty list" in rec.message.lower()
        for rec in caplog.records
...
# Explicit origin + unset FUSION_MLX_CORS_ALLOW_CREDENTIALS must not flip
...
def test_credentials_default_false_with_explicit_origin(
    ...
    """When ``FUSION_MLX_CORS_ALLOW_ORIGINS`` is set to a real origin and
    ``FUSION_MLX_CORS_ALLOW_CREDENTIALS`` is unset, the resolver must not
...
def test_credentials_opt_in_via_env(
    ...
    """Setting ``FUSION_MLX_CORS_ALLOW_CREDENTIALS=true`` enables the
    ``Access-Control-Allow-Credentials: true`` response header so
    cookie / Authorization-bearing fetches succeed."""
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_ORIGINS", "https://chat.openai.com")
    monkeypatch.setenv("FUSION_MLX_CORS_ALLOW_CREDENTIALS", "true")
    _server_mod().configure_cors_from_env(cli_origins=None)
...
def test_cli_origins_path_keeps_legacy_wide_open_headers(
    ...
    monkeypatch.delenv("FUSION_MLX_CORS_ALLOW_METHODS", raising=False)
    monkeypatch.delenv("FUSION_MLX_CORS_ALLOW_HEADERS", raising=False)
...
def test_env_origins_path_applies_f091_narrowing(
    ...
    (``FUSION_MLX_CORS_ALLOW_ORIGINS``), the F-091 narrowing kicks in:
...
def test_legacy_single_arg_configure_cors_keeps_wide_open_headers(
    ...
    """The legacy single-arg back-compat path (``configure_cors(origins)``)
    was retired in #641; ``configure_cors_from_env(cli_origins=...)`` is its
    replacement and is what tests / ``share`` CLI / dflash integration now
    call. Codex round-2 flagged that silently narrowing the defaults would
    break browser clients that send custom headers (``OpenAI-Organization``,
    ``X-Requested-With``, etc.). #641 kept ``allow_headers=["*"]`` on every
    mount path, so the wide-open-header contract is preserved through the
    new entry point."""
    _server_mod().configure_cors_from_env(cli_origins=["https://chat.openai.com"])
