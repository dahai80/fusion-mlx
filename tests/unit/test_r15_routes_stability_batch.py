...
class TestHealthzDrainStateFastPath:
    ...
    def test_fastpath_payload_builder_flips_to_503_when_draining(self):
        from fusion_mlx.middleware.probe_fastpath import _build_healthz_payload
...
class TestResponsesUnknownRole:
    ...
    def test_unknown_role_rejected_by_validator(self):
        ...
        from fusion_mlx.api.responses_models import ResponsesInputItem
...
def _build_completions_app(patch_cfg, monkeypatch, *, engine_factory=None):
    ...
    pytest.skip("routes_internal/completions.py removed (#71 dedup); rewrite for api/openai_routes")
...
def _make_chat_client(engine) -> TestClient:
    ...
    pytest.skip("routes_internal/chat.py removed (#71 dedup); rewrite for api/openai_routes")
