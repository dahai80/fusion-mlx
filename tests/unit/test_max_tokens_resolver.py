...
def test_request_explicit_max_tokens_is_hard_cap_for_thinking_model():
    from fusion_mlx.service.helpers import _resolve_max_tokens
...
@pytest.mark.asyncio
@pytest.mark.xfail(
    reason="strict=False: monkeypatches routes_internal shim; needs update for api/ routes"
)
...
@pytest.mark.xfail(
    strict=False,
    reason="Monkeypatches routes_internal shim; needs update for api/ routes (strict=False)",
)
