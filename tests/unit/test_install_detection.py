...
from fusion_mlx.utils.install import (
...
class TestInstallDetection:
    def test_not_app_bundle_in_dev(self):
        ...
        assert get_cli_prefix() == "fusion-mlx"
    ...
    def test_custom_app_location(self, tmp_path):
        ...
        fake = "/Users/me/Apps/FusionMLX.app/Contents/Resources/fusion_mlx/utils/install.py"
        with (
            patch("fusion_mlx.utils.install.__file__", fake),
            patch("fusion_mlx.utils.install.Path.home", return_value=tmp_path),
            patch("fusion_mlx.utils.install.shutil.which", return_value=None),
    ...
    def test_app_bundle_uses_bare_fusion_when_path_resolves_user_shim(self, tmp_path):
        ...
        with (
            patch("fusion_mlx.utils.install.__file__", fake),
            patch("fusion_mlx.utils.install.Path.home", return_value=tmp_path),
            patch("fusion_mlx.utils.install.shutil.which", return_value=str(shim)),
        ):
            assert get_cli_prefix() == "fusion"

    def test_app_bundle_uses_bare_fusion_when_path_resolves_public_symlink(
        self, tmp_path
    ):
        ...
        shim.write_text("#!/bin/sh\n")
        shim.chmod(0o755)
        public = tmp_path / "bin" / "fusion_mlx"
        public.parent.mkdir()
        public.symlink_to(shim)
        with (
            patch("fusion_mlx.utils.install.__file__", fake),
            patch("fusion_mlx.utils.install.Path.home", return_value=tmp_path),
            patch("fusion_mlx.utils.install.shutil.which", return_value=str(public)),
        ):
            assert get_cli_prefix() == "fusion"

    def test_cli_command_prefix_quotes_full_app_path(self, tmp_path):
        fake = "/Users/me/My Apps/FusionMLX.app/Contents/Resources/fusion_mlx/utils/install.py"
        with (
            patch("fusion_mlx.utils.install.__file__", fake),
            patch("fusion_mlx.utils.install.Path.home", return_value=tmp_path),
            patch("fusion_mlx.utils.install.shutil.which", return_value=None),
            ...
            assert (
                get_cli_command_prefix()
                == "'/Users/me/My Apps/FusionMLX.app/Contents/MacOS/fusion-cli'"
            )
...
class TestIsHomebrew:
    ...
    def test_cellar_prefix(self):
        ...
        with patch("fusion_mlx.utils.install.sys") as mock_sys:
            mock_sys.prefix = "/opt/homebrew/Cellar/fusion-mlx/0.3.0/libexec"
            assert is_homebrew()

    def test_homebrew_prefix(self):
        """Generic /homebrew/ path detected."""
        with patch("fusion_mlx.utils.install.sys") as mock_sys:
            mock_sys.prefix = "/usr/local/homebrew/opt/fusion-mlx/libexec"
            assert is_homebrew()
