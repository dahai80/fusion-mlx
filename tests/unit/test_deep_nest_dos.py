...
* :class:`vllm_mlx.api.models.ToolDefinition` rejects a deeply-nested
  ...
  ``FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH``, default 64) with the canonical
...
* :class:`vllm_mlx.middleware.body_depth.RequestBodyDepthMiddleware`
  ...
  ever recurses over it (envvar ``FUSION_MLX_MAX_BODY_DEPTH``, default
...
def _isolate_env(monkeypatch):
    ...
    monkeypatch.delenv("FUSION_MLX_MAX_BODY_DEPTH", raising=False)
    monkeypatch.delenv("FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH", raising=False)
    yield


from fusion_mlx.api.models import ChatCompletionRequest as _ChatCompletionRequest
...
def _build_minimal_app(*, with_pydantic_chat: bool = False) -> FastAPI:
    ...
    from fusion_mlx.middleware.body_depth import (
...
def test_d_deep_json_depth_1000_returns_400_with_canonical_envelope(path):
    ...
    # NOT be reflected.
    assert "FUSION_MLX_MAX_BODY_DEPTH" in err["message"]
...
def test_d_deep_json_env_override_takes_effect(monkeypatch):
    ...
    ``FUSION_MLX_MAX_BODY_DEPTH``. Setting it tighter MUST reject
    ...
    lookup pattern mirrors ``FUSION_MLX_MAX_REQUEST_BYTES``."""
    app = _build_minimal_app()
    client = TestClient(app, raise_server_exceptions=False)
    ...
    monkeypatch.setenv("FUSION_MLX_MAX_BODY_DEPTH", "500")
...
def test_d_deep_json_disabled_cap_passes_through(monkeypatch):
    """``FUSION_MLX_MAX_BODY_DEPTH=0`` MUST disable the gate — the
    ...
    monkeypatch.setenv("FUSION_MLX_MAX_BODY_DEPTH", "0")
    app = _build_minimal_app()
    client = TestClient(app, raise_server_exceptions=False)
...
def test_d_tool_recur_tools_depth_1000_returns_400_canonical_envelope(monkeypatch):
    ...
    assert "FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH" in err["message"]
...
def test_walk_tools_iter_preserves_nested_tuples():
    ...
    from fusion_mlx.utils.chat_template import _walk_tools_iter
...
def test_d_tool_recur_tool_schema_env_override(monkeypatch):
    ...
    ``FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH``. Mirrors the body-depth env
    override behaviour."""
    monkeypatch.setenv("FUSION_MLX_MAX_BODY_DEPTH", "0")
    monkeypatch.setenv("FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH", "10")
    ...
    assert "FUSION_MLX_MAX_TOOL_SCHEMA_DEPTH" in resp.json()["error"]["message"]
...
def test_body_depth_gate_catches_parser_recursion_error(monkeypatch):
    ...
    import fusion_mlx.middleware.body_depth as _bd
    ...
    assert "FUSION_MLX_MAX_BODY_DEPTH" in body["error"]["message"]
...
def test_quick_depth_heuristic_skips_shallow_bodies():
    ...
    from fusion_mlx.middleware.body_depth import _quick_depth_might_exceed
...
def test_quick_depth_heuristic_string_content_does_not_force_fallback():
    ...
    os.environ.pop("FUSION_MLX_MAX_BODY_DEPTH", None)
    app = _build_minimal_app()
    client = TestClient(app, raise_server_exceptions=False)
    resp = client.post(
...
def test_json_nesting_depth_exceeds_basic():
    from fusion_mlx.utils.json_depth import json_nesting_depth_exceeds
...
def test_resolve_max_helpers_fallback():
    ...
    from fusion_mlx.utils.json_depth import (
    ...
    # Empty env → default.
    os.environ["FUSION_MLX_MAX_BODY_DEPTH"] = ""
    assert resolve_max_body_depth() == DEFAULT_MAX_BODY_DEPTH
    # Garbage env → default.
    os.environ["FUSION_MLX_MAX_BODY_DEPTH"] = "not-a-number"
    assert resolve_max_body_depth() == DEFAULT_MAX_BODY_DEPTH
    # Cleanup.
    del os.environ["FUSION_MLX_MAX_BODY_DEPTH"]
...
def test_body_depth_replay_with_disconnect_preserves_disconnect_semantics():
    ...
    from fusion_mlx.middleware.body_depth import _replay_with_disconnect
...
def test_body_depth_replay_buffered_delegates_to_original_receive_after_body():
    ...
    from fusion_mlx.middleware.body_depth import _replay_buffered
...
def test_body_depth_skips_unguarded_paths(monkeypatch):
    ...
    monkeypatch.setenv("FUSION_MLX_MAX_BODY_DEPTH", "5")
    from fusion_mlx.middleware.body_depth import install_request_body_depth_middleware
    from fusion_mlx.middleware.exception_handlers import install_exception_handlers
