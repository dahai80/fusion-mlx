...
from fusion_mlx.video.minimax_h3.generate import generate_fl2va_video


def test_generate_fl2va_video_accepts_inpaint_params_default_none():
    sig = inspect.signature(generate_fl2va_video)
...
def test_generate_fl2va_video_calls_apply_inpaint_mask():
    src = inspect.getsource(generate_fl2va_video)
    assert "apply_inpaint_mask(gen_latents, init_latent, inpaint_mask)" in src
    assert "inpaint_mask is not None" in src


def test_generate_fl2va_video_re_composite_after_step():
