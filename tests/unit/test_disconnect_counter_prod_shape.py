...
class _BatchedEngineLike:
    ...
    ``_engine`` (the AsyncEngineCore). Does NOT expose ``.scheduler`` directly.
...
def test_sync_scheduler_resolver_finds_deep_prod_path():
    ...
    """
    from fusion_mlx.service.helpers import _resolve_sync_scheduler_for_abort
...
def test_disconnect_recorder_resolver_finds_deep_prod_path():
    ...
    """
    from fusion_mlx.service.helpers import _resolve_disconnect_abort_recorder
...
def test_unresolved_engine_shape_logs_explicit_warning(caplog):
    ...
    class _NakedEngineForWarningTest:
        # No .scheduler, no _engine — the future
