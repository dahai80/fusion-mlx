...
class TestPendingWriteBuffer:
        ...
        try:
            ...
            with mgr._pending_write_hashes_lock:
                for bh in [
                    ...
                    assert (
                        bh not in mgr._pending_writes
                    ), f"{bh!r} still in _pending_writes after writer cleanup"
                    assert (
                        bh not in mgr._pending_write_hashes
    ...
    def test_delete_block_clears_pending_buffer(self, tmp_path):
        ...
        try:
            ...
            assert loaded is None, "Deleted block should not be readable"
        finally:
            mgr.close()
    ...
    def test_queue_full_inline_fallback_cleans_pending_buffer(self, tmp_path):
        ...
        try:
            ...
            with mgr._pending_write_hashes_lock:
                assert (
                    b"qf_test_block_01" not in mgr._pending_writes
