...
``FUSION_REASONING_CUTOFF_NOTICE=disabled`` set, every transport
...
def _opt_out_env(monkeypatch):
    ...
    explicit-opt-out branch (``FUSION_REASONING_CUTOFF_NOTICE=disabled``).
...
def test_helper_returns_none_on_opt_out_env(monkeypatch):
    ...
    ``FUSION_REASONING_CUTOFF_NOTICE=disabled`` is set (the only way
    ...
    from fusion_mlx.service.helpers import _apply_reasoning_cutoff_notice
