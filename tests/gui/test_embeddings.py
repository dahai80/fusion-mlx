...
class EmbeddingTestSuite:
    ...
    async def test_embedding_model(
        ...
        print(
            f"   Architecture: {spec.architecture} | Expected Dims: {spec.expected_dims} | {spec.quantization}"
        )
        ...
        try:
            ...
            # Print results
            print(
                f"   ✅ Generated {len(embeddings_data)} embeddings in {response_time:.2f}s"
            )
            print(
                f"   📊 Dimensions: {actual_dims} (expected: {spec.expected_dims}) {'✅' if test_result.dims_correct else '❌'}"
            )
            print(
                f"   📊 Value Range: {value_range[0]:.3f} to {value_range[1]:.3f} {'✅' if test_result.reasonable_range else '⚠️'}"
            )
            print(
                f"   📊 Vector Norm: {vector_norm:.3f} {'✅' if test_result.normalized else '⚠️'}"
            )
            print(f"   📊 Mean: {mean_value:.3f}, Std: {std_value:.3f}")
            print(f"   📊 Sample: {sample_values}")

            if (
                test_result.dims_correct
                and test_result.normalized
                and test_result.reasonable_range
            ):
    ...
    async def run_comprehensive_test(self, test_models: list[str] | None = None):
    ...
    def print_summary(self):
        ...
        if successful:
            ...
            for result in successful:
                ...
                print(
                    f"      Dims: {result.actual_dims}, Norm: {result.vector_norm:.3f}, Range: {result.value_range[0]:.3f} to {result.value_range[1]:.3f}"
                )
