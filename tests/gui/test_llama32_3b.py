    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        try:
            response = await client.get(
                f"{BASE_URL}/v1/discover/models?query=Llama-3.2"
            )
        ...
        try:
            install_response = await client.post(
                ...
                json={"model_id": MODEL_ID, "name": "llama-32-3b-instruct-4bit"},
            )
        ...
        instruction_tests = [
            ...
            },
            {
                ...
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": "How can I improve my time management skills? Give me 3 specific strategies.",
                    },
                ],
            },
            {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a technical education assistant.",
                    },
                    {
                        "role": "user",
        ...
        try:
            conversation_messages = [
                ...
                {
                    "role": "user",
                    "content": "I'm planning a trip to Japan. What are some must-see places?",
                },
            ]
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    if (
                        "japan" in food_response.lower()
                        or "japanese" in food_response.lower()
                    ):
        ...
        try:
            stream_request = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are an expert software architect. Provide detailed, structured responses.",
                    },
                    {
                        "role": "user",
                        "content": "Design a microservices architecture for an e-commerce platform. Include the main services and their responsibilities.",
                    },
                ],
                "max_tokens": 400,
                "temperature": 0.4,
        ...
        for case in edge_cases:
            try:
                ...
                response = await client.post(
                    f"{BASE_URL}/v1/chat/completions", json=safety_request
                )
                if response.status_code == 200:
                    result = response.json()
                    content = result["choices"][0]["message"]["content"]
        ...
        try:
            health_response = await client.get(
                f"{BASE_URL}/v1/models/llama-32-3b-instruct-4bit/health"
            )
            if health_response.status_code == 200:
                health_data = health_response.json()
                print(f"   ✅ Model health: {health_data}")
                print("   📈 Model characteristics:")
                print("      - Parameters: 502M (3B architecture)")
                print("      - Quantization: 4-bit")
                print("      - Downloads: 14,332/month")
                print("      - Specialization: Instruction following")
            else:
