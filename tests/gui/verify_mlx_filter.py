...
def test_mlx_library_filter():
    ...
    try:
        ...
        models = list(
            list_models(library="mlx", limit=5, sort="downloads", direction=-1)
        )
        for model in models:
            print(
                f"  - {model.id} (library: {getattr(model, 'library_name', 'unknown')})"
            )
        ...
        models = list(
            list_models(tags=["mlx"], limit=5, sort="downloads", direction=-1)
        )
        for model in models:
        ...
        for model_id in known_mlx_models:
            try:
                ...
                print(
                    f"    MLX in tags: {'mlx' in [tag.lower() for tag in tags] if tags else False}"
                )
