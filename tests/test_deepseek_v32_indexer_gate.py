...
``fusion_mlx.patches.deepseek_v32_indexer_gate`` for REAP-pruned DeepseekV32
...
try:
    ...
    _HAS_INDEXER_GATE = False


def _require_indexer_gate():
    if not _HAS_INDEXER_GATE:
        pytest.skip("fusion_mlx.patches.deepseek_v32_indexer_gate not migrated yet")
...
def test_upstream_without_gate_fails_with_missing_indexer_keys(monkeypatch, repro_dir):
    ...
    uninstall_deepseek_v32_indexer_gate()
    monkeypatch.setattr(
        "fusion_mlx.patches.deepseek_v32_indexer_gate._INSTALLED",
        False,
        raising=False,
    )
...
def test_gate_loads_mixed_full_shared_config(repro_dir):
    ...
    4. Forward output is finite (no NaN/Inf) on both the reuse and
       dense paths.
    """
    from fusion_mlx.patches import deepseek_v32_indexer_gate as gate
...
def test_install_fires_on_real_serve_import_path():
    ...
    ``fusion_mlx/model_runner.py:34``. The real ``fusion-mlx serve`` boot
    path is::
    ...
    directly via ``from fusion_mlx.patches.deepseek_v32_indexer_gate import
    ...
    (``del sys.modules["fusion_mlx..."]``) pollutes the test session because
    ...
    script = textwrap.dedent(
        ...
        import fusion_mlx.utils.tokenizer  # noqa: F401

        # Read the post-install state via the patch module's public API
        # plus the upstream-class marker (belt + suspenders).
        from fusion_mlx.patches.deepseek_v32_indexer_gate import is_installed
