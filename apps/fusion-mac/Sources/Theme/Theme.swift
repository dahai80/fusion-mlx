...
extension FusionTheme {
    static let light = FusionTheme(
        isDark: false,
...
private struct FusionThemeKey: EnvironmentKey {
    static let defaultValue: FusionTheme = .light
}

extension EnvironmentValues {
    var fusionTheme: FusionTheme {
        get { self[FusionThemeKey.self] }
        set { self[FusionThemeKey.self] = newValue }
    }
}

private struct FusionThemeBinder: ViewModifier {
    @Environment(\.colorScheme) private var scheme
    func body(content: Content) -> some View {
        content.environment(\.fusionTheme, scheme == .dark ? .dark : .light)
    }
}

extension View {
    /// Resolves `\.fusionTheme` from the current `\.colorScheme`. Apply once at
    /// the AppView shell (PR 6) so every descendant primitive reads the right
    /// palette without explicit prop-drilling.
    func fusionThemed() -> some View { modifier(FusionThemeBinder()) }
}
...
extension Font {
    ...
    static func fusionText(_ size: CGFloat, weight: Font.Weight = .regular) -> Font {
        .system(size: size, weight: weight)
    }
    /// Display weight for headlines (size auto-promotes on macOS at ≥ 20pt).
    static func fusionDisplay(_ size: CGFloat, weight: Font.Weight = .semibold) -> Font {
        .system(size: size, weight: weight)
    }
    /// Monospaced (SF Mono fallback chain).
    static func fusionMono(_ size: CGFloat, weight: Font.Weight = .regular) -> Font {
