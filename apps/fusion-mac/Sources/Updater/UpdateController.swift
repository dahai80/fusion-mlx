...
// `~/Library/Application Support/FusionMLX/update-prefs.json` so they survive
// a relaunch.
//
...
final class UpdateController {
    static let stateDidChangeNotification = Notification.Name("FusionUpdateControllerStateDidChange")
    ...
    private func runCheck(userInitiated: Bool) async {
        do {
            ...
            NSLog("FusionMLX: update check failed — %@", String(describing: error))
            await MainActor.run {
                if userInitiated {
