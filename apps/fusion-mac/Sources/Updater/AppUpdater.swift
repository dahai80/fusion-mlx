...
//
// Flow: download .dmg → hdiutil attach → copy the inner FusionMLX.app next to
// the running bundle as `.FusionMLX-update.app` → hdiutil detach → on
...
final class AppUpdater {
    enum UpdateError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case .notWritable(let path):
                return "Cannot write to \(path). Move FusionMLX.app to a writable location and try again."
            case .downloadFailed(let m): return "Download failed: \(m)"
            case .mountFailed(let m): return "Could not mount DMG: \(m)"
            case .appNotFoundInVolume: return "FusionMLX.app not found inside the downloaded DMG"
    ...
    static let stagedAppName = ".FusionMLX-update.app"
    ...
    private func run(app: URL) async {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fusion-update-\(UUID().uuidString)")
        do {
        ...
        let dmgPath = tmpDir.appendingPathComponent("FusionMLX-\(version).dmg")
    ...
    private func findAppInVolume(_ mountPoint: URL) throws -> URL {
        let preferred = mountPoint.appendingPathComponent("FusionMLX.app")
    ...
    static func performSwapAndRelaunch() -> Bool {
        ...
        do {
            ...
            NSLog("FusionMLX: failed to spawn swap script: %@", error.localizedDescription)
            return false
        }
        return true
    }
