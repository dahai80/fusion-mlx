...
// `Window` scene declared in FusionMLXApp.swift — we no longer build it
// manually here.
//
// Boot flow
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Quit / "Quit FusionMLX" from the application menu all route through
    ...
    /// `fusionapp://main` URL — the Window scene in FusionMLXApp.swift handles it
    ...
    func presentAppView() {
        ...
        if let url = URL(string: "fusionapp://main") {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !isRunningUnitTests {
            do {
                ...
                NSLog("FusionMLX: CLI shim setup failed — \(error)")
            }
            startControlServer()
        }
        ...
        if AppConfig.hasExistingConfig {
            ...
            // file (`~/.fusion-mlx/settings.json` first, Library config.json
    ...
    private func promptForShellPathExport(reason: String) {
        let alert = NSAlert()
        alert.messageText = "Enable `fusion` in Terminal?"
        alert.informativeText = """
        FusionMLX could not create a public `fusion` command in /opt/homebrew/bin or /usr/local/bin.

        To make `fusion` available in new Terminal sessions, FusionMLX can add a small PATH block to your shell init file. This only happens if you choose Update Shell File.
        ...
        case .alertFirstButtonReturn:
            do {
                ...
                NSLog("FusionMLX: CLI shell path setup failed — \(error)")
            }
    ...
    @objc private func windowDidBecomeMainNotification(_ notif: Notification) {
        ...
        // (the "FusionMLX" floating above the toolbar zone). The title string
    ...
    /// The single real-quit path is the menubar status item's "Quit FusionMLX",
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        // Same close path used by the SwiftUI Cmd-Q command in FusionMLXApp.swift.
    ...
    private func startControlServer() {
        ...
        do {
            ...
            NSLog("FusionMLX: app-control server failed to start — \(error)")
        }
    }
    ...
    func handleAppControl(_ command: AppControlServer.Command) async -> AppControlServer.Response {
        guard let server else {
            return .failure(
                ...
                state: .stopped,
                server: nil,
                message: "Managed server is unavailable. Complete the FusionMLX first-run setup in the app."
            )
        }
