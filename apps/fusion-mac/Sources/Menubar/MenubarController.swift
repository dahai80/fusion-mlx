...
final class MenubarController: NSObject {
    ...
    init(
        ...
        if statusItem.button?.image == nil {
            let fallback = NSImage(
                ...
                accessibilityDescription: "FusionMLX"
            )
    ...
    private func buildMenu() {
        ...
        chatItem = item(String(localized: "menubar.item.chat",
                               defaultValue: "Chat with FusionMLX",
    ...
    private func refreshUpdateMenuItem() {
        ...
        case .available(let info), .ready(let info):
            ...
            updateItem.title = String(localized: "menubar.item.install_update_version",
                                      defaultValue: "Install FusionMLX \(info.version)…",
        ...
        case .stopped:
            ...
            return (
                String(localized: "menubar.header.stopped",
                       defaultValue: "FusionMLX stopped",
        ...
        if session == nil && alltime == nil {
            statsSubmenu.addItem(disabled(statsPoller == nil
                                          ? String(localized: "menubar.stats.no_api_key",
                                                   defaultValue: "Set Fusion_API_KEY to enable stats",
    ...
    @objc private func serverPortConflict(_ note: Notification) {
        ...
        let key = "\(conflict.pid.map { String($0) } ?? "unknown"):\(conflict.isFusion)"
    ...
    @objc private func startServer() {
        ...
        do {
            ...
            NSLog("FusionMLX: start failed — \(error)")
        }
    }
    ...
    @objc private func forceRestartServer() {
        ...
        Task { @MainActor in
            do {
                ...
                NSLog("FusionMLX: force-restart failed — \(error)")
            }
        }
    }
    ...
    private func presentPortConflictAlert(_ conflict: PortConflict) {
        ...
        alert.informativeText = conflict.isFusion
            ? String(localized: "menubar.alert.port_in_use.fusion",
                     defaultValue: "Another FusionMLX server is already running on this port (\(pidStr)). Stop it before starting a new instance, or change the port in Settings.",
                     comment: "Port-conflict alert body when the conflicting process is another FusionMLX instance")
    ...
    private func presentServerFailureAlert(message: String, logURL: URL) {
        ...
        alert.messageText = String(localized: "menubar.alert.server_failed.title",
                                   defaultValue: "FusionMLX Server Failed to Start",
        ...
        alert.addButton(withTitle: String(localized: "menubar.alert.open_settings",
                                          ...
                                          comment: "Button that opens the FusionMLX settings window"))
...
extension MenubarController {
    ...
    static func accessFailureHint(message: String, logTail: String?) -> String? {
        ...
        return String(localized: "menubar.alert.server_failed.access_hint",
                      defaultValue: "Check that the configured model directory is mounted and readable. If it is on an external or protected location, grant FusionMLX access in macOS Privacy & Security settings.",
