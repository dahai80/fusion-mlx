...
final class MenubarVisibilityWatcher {
    ...
    private let logURL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Application Support/FusionMLX/logs/menubar.log")
    ...
    private func showHiddenAlert() {
        ...
        let alert = NSAlert()
        alert.messageText = "FusionMLX Menubar Icon Hidden"

        if isTahoeOrNewer {
            alert.informativeText = """
            The FusionMLX menubar icon isn't showing up.
            ...
            FusionMLX and restart ControlCenter. It needs Full Disk Access.

            You can also enable FusionMLX manually in System Settings > Menu Bar.
            """
            ...
            third-party menubar apps. Try quitting and relaunching FusionMLX, \
    ...
    private func fixStatusKitPermission() -> AutoFixOutcome {
        ...
        let primaryBundleID = Bundle.main.bundleIdentifier ?? "app.fusion-mlx"
        let targetBundleIDs = Set([primaryBundleID, "app.fusion-mlx", "com.fusion-mlx.app"])
        ...
        for var entry in entries {
            ...
            guard let bundleID = locationBundleID, targetBundleIDs.contains(bundleID) else {
                ...
                // menuItemLocations=FusionMLX. ControlCenter may keep those around
                // after bundle-id changes, but they do not back the FusionMLX toggle.
                changed = true
                continue
            }
        ...
        if !changed {
            ...
            return AutoFixOutcome(
                success: true,
                message: """
                FusionMLX is already approved in StatusKit (\(knownIDs)). If the \
        ...
        if !restartControlCenter() {
            return AutoFixOutcome(
                success: true,
                message: """
                StatusKit was updated but FusionMLX couldn't restart ControlCenter. \
    ...
    private func backupStatusKitPlist() -> URL? {
        ...
        let backupDirectory = fileManager.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/FusionMLX/backups")
    ...
    private func writeStatusKitPlist(_ data: Data, backup: URL?) throws {
        ...
        let temporaryURL = statusKitPlistURL
            ...
            .appendingPathComponent(statusKitPlistURL.lastPathComponent + ".fusion-tmp")
    ...
    private func showStatusKitAccessDeniedAlert() {
        ...
        Enable FusionMLX in System Settings > Privacy & Security > Full Disk \
