...
//   1. FUSION_BASE_PATH env var. macOS Finder/Dock launches inherit only the
...
//   2. ~/Library/Application Support/FusionMLX/base-path — a one-line
...
struct AppConfig: Sendable, Equatable, Codable {
    ...
    var apiKey: String?
    /// Always `FUSION_BASE_PATH` if set, else `~/.fusion-mlx`. Set at load() time
    ...
    /// Resolve the user-effective base path. `FUSION_BASE_PATH` env wins,
    /// then the bootstrap file under Library/Application Support, then the
    /// `~/.fusion-mlx` default.
    static func currentBasePath() -> String {
        let env = ProcessInfo.processInfo.environment
        if let custom = env["FUSION_BASE_PATH"], !custom.isEmpty {
    ...
    /// has no `FUSION_BASE_PATH`. The single field it carries is the path
    ...
    /// "reset to ~/.fusion-mlx default" flow. Callers should compare against
    ...
    static func defaultBasePath() -> String {
        ...
        return home.appendingPathComponent(".fusion-mlx", isDirectory: true).path
    }
    ...
    static func appSupportURL() -> URL {
        ...
        let dir = base.appendingPathComponent("Fusion-MLX", isDirectory: true)
    ...
    static func load() -> AppConfig {
        ...
        // via currentBasePath().
        let env = ProcessInfo.processInfo.environment
        if let h = env["FUSION_HOST"], !h.isEmpty { c.bindAddress = h }
        if let pStr = env["FUSION_PORT"], let p = Int(pStr) { c.port = p }
        if let k = env["FUSION_API_KEY"], !k.isEmpty { c.apiKey = k }
        return c
    }
