...
//   1. Fusion_PYTHON_OVERRIDE env var — dev escape hatch.
...
//      apps/fusion-mac/Scripts/build.sh copies verbatim into the Swift .app.
...
//     so `python -m fusion.cli` resolves both the fusion package (shipped as a
//     pure source tree in Resources/fusion/, matching today's Python build)
...
struct PythonRuntime {
    ...
    static func resolve() throws -> PythonRuntime {
        ...
        if let override = env["Fusion_PYTHON_OVERRIDE"], !override.isEmpty {
