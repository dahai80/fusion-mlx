// Lifecycle owner for the `fusion serve` child process.
//
// State machine
...
//
// Spawn invocation:
//   <python> -m fusion_mlx.cli serve --base-path <base> --port <port>
//   stdout+stderr → ~/Library/Application Support/FusionMLX/logs/server.log
//   PATH = parent + Homebrew prefixes
//
// Dev override: FUSION_DEV_SERVER_SCRIPT spawns <python> <script> --port …
// instead, used by Scripts/dev_server.py to exercise the spawn path without
// the full fusion stack.
//
...
final class ServerProcess: @unchecked Sendable {
    ...
    static let stateDidChangeNotification = Notification.Name("FusionServerProcessStateDidChange")
    static let portConflictNotification   = Notification.Name("FusionServerPortConflict")
    ...
    func start() throws -> StartResult {
        ...
        if resolver.isPortInUseSync() {
            let conflict = PortConflict(
                ...
                isFusion: resolver.isFusionOnPortSync()
            )
            update(.failed(message: "Port \(port) in use" +
                           (conflict.isFusion ? " (FusionMLX server already running)" : "")))
    ...
    private func tryAutoRestart(reason: String) {
        ...
        NSLog("FusionMLX: auto-restart \(attempt)/\(maxAutoRestarts) in \(Int(backoff))s — \(reason)")
        update(.starting)
    ...
    private func logHealthProbeFailure(
        ...
        NSLog(
            "FusionMLX: health probe failed url=\(result.url) latency_ms=\(result.latencyMs) status=\(status) error=\(error) failures=\(failures) suppressed_by_recent_status=\(suppressed)"
        )
    }
    ...
    private func makeArguments() -> [String] {
        ...
        if let dev = env["FUSION_DEV_SERVER_SCRIPT"], !dev.isEmpty {
            return [dev, "--host", bindAddress, "--port", String(port)]
        }
        return [
            "-m", "fusion_mlx.cli", "serve",
