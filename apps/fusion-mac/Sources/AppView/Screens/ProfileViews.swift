...
// on the Server screen. The shapes mirror fusion-screens.jsx:626-1191:
//
...
/// Per-scope visual treatment. Lifted from fusion-screens.jsx:878-884
/// (SCOPE_META) so the chip dots and badges read the same as the canvas.
enum ProfileScopeMeta {
    static func color(_ scope: ProfileScope, theme: FusionTheme) -> Color {
        switch scope {
...
struct ProfileGroup: View {
    ...
    /// fusion.ai via `POST /api/presets/refresh`.
        ...
        HStack(spacing: 8) {
            ...
            if let onRefresh {
                ...
                .help(String(localized: "profile.group.refresh.help",
                             defaultValue: "Refresh presets from fusion.ai",
    ...
    /// (`fusion/model_profiles.py:validate_profile_name`). Pre-checking
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(titleText)
                    .font(.fusionText(13.5, weight: .semibold))
...
struct ProfileDetailCard: View {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(name)
                        .font(.fusionText(16, weight: .semibold))
                        .foregroundStyle(theme.text)
    ...
    private var sections: some View {
        ...
        // `MODEL_SPECIFIC_PROFILE_FIELDS` in `fusion/model_profiles.py`.
    ...
    private func specprefillLabel(_ s: [String: AnyCodable], on: Bool) -> String {
        ...
        // `fusion/model_settings.py:62`); render as a percent so the chip
        // reads naturally.
    ...
    private func sectionTitle(_ s: String) -> some View {
        Text(s)
            .font(.fusionText(10, weight: .heavy))
            .kerning(0.9)
            .textCase(.uppercase)
            .foregroundStyle(theme.textTertiary)
            .padding(.bottom, 2)
        ...
        VStack(alignment: .leading, spacing: 5) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Text(String(format: fmt, value))
                    .font(.fusionMono(12.5, weight: .semibold))
                    .foregroundStyle(theme.text)
            }
    ...
    private func stat(label: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(label)
                ...
                .foregroundStyle(theme.textSecondary)
            Text(value)
                .font(.fusionMono(14, weight: .semibold))
                .foregroundStyle(theme.text)
                .lineLimit(1)
                .truncationMode(.tail)
        }
    ...
    private func badge(text: String, fg: Color, bg: Color, border: Color? = nil) -> some View {
        Text(text)
            .font(.fusionText(9.5, weight: .heavy))
            .kerning(0.7)
            .foregroundStyle(fg)
