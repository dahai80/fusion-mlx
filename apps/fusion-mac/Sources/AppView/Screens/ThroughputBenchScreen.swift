...
// (fusion/admin/templates/dashboard/_bench.html + dashboard.js benchmark
...
private struct SingleResultsTable: View {
            ...
            FreeRow {
                HStack(spacing: 10) {
                    ForEach(Array(columnHeaders.enumerated()), id: \.offset) { _, h in
                        Text(h)
                            .font(.fusionText(10.5, weight: .semibold))
                            .foregroundStyle(theme.textTertiary)
                            .textCase(.uppercase)
                            .kerning(0.4)
    ...
    private func cell(_ text: String, mono: Bool = false) -> some View {
        Text(text)
            .font(mono ? .fusionMono(11.5) : .fusionText(11.5))
            .foregroundStyle(theme.text)
            .lineLimit(1)
            .frame(maxWidth: .infinity, alignment: .leading)
    }
...
/// Renders the result of the post-bench upload to the public fusion.ai
/// leaderboard. The upload happens server-side automatically after every
/// run (fusion/admin/benchmark.py:_upload_to_fusion_ai); we just surface the
/// state. Three modes:
                ...
                FreeRow(isLast: true) {
                    HStack(spacing: 10) {
                        ...
                        Text(String(localized: "bench.throughput.upload.uploading",
                                    defaultValue: "Uploading to fusion.ai…",
