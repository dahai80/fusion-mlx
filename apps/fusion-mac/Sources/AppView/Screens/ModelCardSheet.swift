...
//   • HF       → FusionClient.getHFModelCard(repoId:)
//   • MS       → FusionClient.getMSModelCard(modelId:)
//
...
struct ModelCardSheet: View {
    ...
    private func tabButton(tab: ModelCardTab, isSelected: Bool) -> some View {
        Button {
            ...
            Text(tab.label)
                .font(.fusionText(11.5, weight: .semibold))
    ...
    private var loraBanner: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            Text(String(localized: "downloads.card.lora_warning",
                        defaultValue: "This is a LoRA adapter. It needs a compatible base model to run — downloading on its own won't load in FusionMLX.",
    ...
    private func load() async {
        ...
        do {
            ...
            state = .failed(error.fusionDescription)
        }
    }
