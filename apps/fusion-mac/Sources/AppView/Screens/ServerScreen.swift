...
struct ServerScreen: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ListGroup {
                ...
                Row(
                    ...
                    sublabel: String(localized: "server.row.port.sub",
                                     defaultValue: "Default 11435. Server restarts on save.",
            ...
            SectionHeader(String(localized: "server.section.appearance",
                                  defaultValue: "Appearance",
                                  comment: "Section heading for UI appearance settings"))
            ListGroup {
                Row(label: String(localized: "server.row.ui_language",
                                  defaultValue: "Language",
                                  comment: "Row label for the UI language picker"),
                    sublabel: String(localized: "server.row.ui_language.sub",
                                     defaultValue: "Display language for the admin panel and server responses.",
                                     comment: "Sublabel for the UI language picker"),
                    isLast: true) {
                    Popup(
                        selection: vm.bind($vm.uiLanguage, save: vm.saveUILanguage),
                        width: 150,
                        options: [
                            ("",    String(localized: "server.ui_language.system",
                                           defaultValue: "System",
                                           comment: "UI language option: follow system locale")),
                            ("en",  "English"),
                            ("zh",  "中文"),
                            ("ja",  "日本語"),
                            ("ko",  "한국어"),
            ...
            ListGroup {
                Row(
                    ...
                    sublabel: String(localized: "server.row.base_path.sub",
                                     defaultValue: "Fusion_BASE_PATH. Files move and the server restarts when this changes.",
    ...
    private func modelDirRow(index: Int) -> some View {
        HStack(spacing: 7) {
            Text(index == 0
                ...
                .font(.fusionMono(10, weight: .semibold))
...
/// Hero card shared between the Server and Status screens (fusion-screens.jsx
...
struct ServerHeroCard: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            // App logo — `AppLogo` asset ships the rounded fusion mark (same
            // artwork as the README's hero icon, light/dark variants).
            // Replaces the previous gradient squircle + "oM" placeholder
            // so the hero card on Server / Status reads as FusionMLX rather
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 10) {
                    Text(title)
                        .font(.fusionText(18, weight: .semibold))
                        .foregroundStyle(theme.text)
    ...
    private var buttons: some View {
        ...
        case .running, .unresponsive:
            HStack(spacing: 6) {
                ...
                .buttonStyle(.fusion(.destructive))
            }
    ...
    private var title: String {
        ...
        return trimmed.isEmpty ? "FusionMLX" : "FusionMLX \(trimmed)"
    }
}
    ...
    private func perModelOnlyRow(label: String, note: String, isLast: Bool = false) -> some View {
        Row(label: label, sublabel: note, isLast: isLast) {
            Text(String(localized: "server.profile.per_model_only",
                ...
                .font(.fusionText(10.5, weight: .heavy))
                .kerning(0.6)
                .textCase(.uppercase)
...
private struct ServerAdvancedSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // doesn't match the rest of the screen (uses SF Pro vs fusionText,
                ...
                ListGroup {
                    ...
                    Row(
                        ...
                        TextInput(
                            ...
                            placeholder: "fusion.local, FusionMLX.lan",
                            mono: true,
                            width: 320
                        )
                    }
