...
struct StatusScreen: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Same hero card the Server screen uses (fusion-screens.jsx:75
            ...
            ListGroup {
                ...
                Row(label: String(localized: "status.row.version",
                                  defaultValue: "FusionMLX Version",
                                  comment: "Row label for the running FusionMLX version"),
                    isLast: true) {
                    Text(vm.versionText)
                        .font(.fusionMono(12))
                ...
                HStack(spacing: 8) {
                    ...
                    if hotTierEnabled {
                        Button(String(localized: "status.runtime_cache.clear_hot",
                            ...
                            .buttonStyle(.fusion(.destructive, size: .small))
...
/// `fusion/admin/static/js/dashboard.js` — 1024-scaled, one decimal place.
    ...
    private var primaryLine: some View {
        ...
        case .available(let upd), .ready(let upd):
            Text(String(localized: "status.updates.available_primary",
                        defaultValue: "FusionMLX \(upd.version) is available",
        ...
        case .idle:
            Text(updates.lastError == nil
                 ? String(localized: "status.updates.up_to_date_primary",
                          defaultValue: "FusionMLX is up to date",
