...
// hosted in the SwiftUI `Window` scene declared in `FusionMLXApp.swift`.
...
struct AppView: View {
    ...
    private func screen(for section: AppSection) -> some View {
        ...
        case .downloads:    DownloadsScreen(vm: services.downloads)
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(String(localized: "update.confirm.title",
                            defaultValue: "FusionMLX \(update.version) is available",
                            comment: "Update confirmation sheet title; placeholder is the version"))
                    .font(.fusionText(17, weight: .semibold))
        ...
        HStack(spacing: 10) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(String(localized: "update.confirm.restart_notice",
                            defaultValue: "FusionMLX will quit, install the update, and relaunch.",
...
/// Mirrors `ContentArea` from the design (fusion-components.jsx:250-292):
