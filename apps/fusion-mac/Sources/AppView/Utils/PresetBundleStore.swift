...
//
//   1. Disk cache (~/Library/Application Support/FusionMLX/preset_cache.json)
//   2. Bundled fixture (Contents/Resources/fusion/admin/static/fusion_preset.json)
//      — staged into the .app by Scripts/build.sh from the fusion package.
//   3. Remote refresh via POST /admin/api/presets/refresh (fusion.ai proxy).
//
...
// retired (see Phase 1 — fusion/template/default_global_profile.json
...
final class PresetBundleStore {
    ...
    /// HTML's `localStorage.fusion_preset_cache`.
    ...
    func refresh(client: FusionClient) async {
        ...
        do {
            ...
            self.lastError = (error as? FusionClientError)
    ...
    private static func defaultCacheURL() -> URL {
        ...
        let dir = base.appendingPathComponent("FusionMLX", isDirectory: true)
    ...
    /// `Contents/Resources/fusion/admin/static/fusion_preset.json`. The file
    /// is staged into the app by `Scripts/build.sh` (fusion package rsync),
    ...
    private static func loadFixture() -> PresetBundleDTO? {
        ...
        let url = resources
            ...
            .appendingPathComponent("fusion_preset.json")
        guard FileManager.default.fileExists(atPath: url.path),
              let data = try? Data(contentsOf: url) else { return nil }
        return try? JSONDecoder().decode(PresetBundleDTO.self, from: data)
    }
