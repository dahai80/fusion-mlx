...
final class AppServices: NSObject {
    ...
    /// via the UI (or CLI) continue showing progress when the user returns
    /// to the Downloads tab. Server-side downloads are independent of the
    /// client; this only controls whether we keep tracking them.
    let downloads = DownloadsScreenVM()
    ...
    init(config: AppConfig = .default, server: ServerProcess? = nil) {
        self.config = config
        self.client = FusionClient(host: config.host, port: config.port, apiKey: config.apiKey)
    ...
    ///     `Fusion_BASE_PATH` (env + bootstrap file), and
    ...
    private func migrateBasePath(to newPath: String) throws {
        ...
        // When the user resets to the `~/.fusion-mlx` default, clear every
        ...
        do {
            ...
            NSLog("FusionMLX: relocateOrphanPaths failed: %@", String(describing: error))
        }
    }
    ...
    nonisolated static func relocateOrphanPaths(in url: URL, oldBase: String, newBase: String) throws {
        NSLog("FusionMLX: relocateOrphanPaths in=%@ old=%@ new=%@",
              url.path, oldBase, newBase)
        guard FileManager.default.fileExists(atPath: url.path) else {
            NSLog("FusionMLX: relocateOrphanPaths skipped — file does not exist")
            return
        }
        let data = try Data(contentsOf: url)
        guard var json = try JSONSerialization.jsonObject(with: data) as? [String: Any]
        else {
            NSLog("FusionMLX: relocateOrphanPaths skipped — root is not an object")
            return
        }
        ...
        NSLog("FusionMLX: relocateOrphanPaths wrote %d bytes", out.count)
    }
