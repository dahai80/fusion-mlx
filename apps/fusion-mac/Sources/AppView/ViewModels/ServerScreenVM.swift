...
final class ServerScreenVM {
    ...
    /// UI display language. Empty string = system default. Persisted
    /// server-side via `ui_language`; the server applies it to i18n
    /// helpers at runtime.
    ...
    private var baselinePortText: String = "11435"
    ...
    private var baselineUiLanguage: String = ""
    ...
    func load(client: FusionClient) async {
        ...
        do {
            ...
            self.uiLanguage = dto.ui?.language ?? ""
    ...
    func saveUILanguage() {
        Task {
            if await commit(GlobalSettingsPatch(uiLanguage: uiLanguage)) {
                self.baselineUiLanguage = self.uiLanguage
            }
        }
    }
