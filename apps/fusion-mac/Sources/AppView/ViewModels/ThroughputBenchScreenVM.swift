...
final class ThroughputBenchScreenVM {
    ...
    func runBenchmark(client: FusionClient) {
    ...
    func cancelBenchmark(client: FusionClient) {
        ...
        Task { [weak self] in
            do {
                ...
                await MainActor.run { self?.lastError = error.fusionDescription }
            }
            await MainActor.run {
                self?.running = false
    ...
    private func pollResults(client: FusionClient) {
        pollTask?.cancel()
        ...
        pollTask = Task { [weak self] in
            while !Task.isCancelled {
                ...
                do {
                    ...
                    // (benchmark.py:_upload_to_fusion_ai) and surfaces it
