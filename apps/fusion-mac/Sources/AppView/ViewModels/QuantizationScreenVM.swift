...
final class QuantizationScreenVM {
    ...
    func scheduleEstimateRefresh(client: FusionClient) {
    ...
    func cancelTask(taskId: String, client: FusionClient) {
        Task { [weak self] in
            do {
    ...
    func removeTask(taskId: String, client: FusionClient) {
        Task { [weak self] in
            do {
    ...
    func validateUploadToken(client: FusionClient) async {
        ...
        do {
            ...
            self.lastUploadError = String(localized: "quant.upload.error.validate_failed",
                                          defaultValue: "Validate failed: \(error.fusionDescription)",
    ...
    func startUpload(body: HFUploadStartRequest, client: FusionClient) async {
        lastUploadError = nil
        do {
            ...
            lastUploadError = String(localized: "quant.upload.error.start_failed",
                                     defaultValue: "Upload failed: \(error.fusionDescription)",
                                     comment: "Error when an upload start request throws. Placeholder is the underlying error description")
        }
    }

    func cancelUpload(taskId: String, client: FusionClient) {
        Task { [weak self] in
            do {
    ...
    func removeUpload(taskId: String, client: FusionClient) {
        Task { [weak self] in
            do {
