...
struct StatsDTO: Codable, Equatable, Sendable {
    ...
    let apiKey: String?
    /// Absolute CLI invocation prefix used for `fusion launch <tool>` strings.
    /// `nil` on older servers; callers fall back to the bare `"fusion"` token
    ...
    /// Mirrors `_build_runtime_cache_observability` in `fusion/admin/routes.py`.
