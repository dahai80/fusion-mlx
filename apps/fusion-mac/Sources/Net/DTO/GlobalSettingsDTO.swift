...
struct GlobalSettingsDTO: Codable, Equatable, Sendable {
    ...
    /// Mirrors `fusion.settings.HuggingFaceSettings`. Empty string means
    ...
    /// Mirrors `fusion.settings.SamplingSettings`. The full server surface
    ...
    /// Mirrors `fusion.settings.MCPSettings`. The server stores a single path to
    ...
    /// Mirrors `fusion.settings.ModelScopeSettings`. Empty string means
    /// "use the default" (modelscope.cn). Patched via `ms_endpoint`.
    struct ModelScopeDTO: Codable, Equatable, Sendable {
        let endpoint: String
    }

    /// Mirrors `fusion.settings.NetworkSettings`. All four fields are simple
    ...
    /// Mirrors `fusion_mlx` `ui.*` block. `language` is a locale code
    /// (`"en"`, `"zh"`, `"ja"`, …) or empty string for system default.
    /// Patched via the flat `ui_language` key on `GlobalSettingsPatch`.
...
struct GlobalSettingsPatch: Encodable, Equatable, Sendable {
    ...
    // see `fusion/admin/routes.py:229-234`. They patch in-place; non-nil fields
    ...
    /// place via `fusion/admin/routes.py:2804`.
    ...
    // applies live wherever possible — see `fusion/admin/routes.py` for the
    ...
    /// UI display language. Locale code (`"en"`, `"zh"`, `"ja"`, …) or
    /// empty string for system default. Encoded as `ui_language`.
    var uiLanguage: String? = nil
}
