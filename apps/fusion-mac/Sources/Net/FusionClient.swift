...
enum FusionClientError: Error, CustomStringConvertible {
    case invalidURL
...
extension Error {
    ...
    /// FusionClientError formats via its `description` (HTTP status + body,
    /// authentication state, etc.); other errors fall back to
    /// `localizedDescription`. Replaces the per-VM `describe(_:)` helper.
    var fusionDescription: String {
        if let fusion = self as? FusionClientError { return String(describing: fusion) }
        return localizedDescription
    }
}

@MainActor
final class FusionClient: ObservableObject {
    ...
    init(host: String = "127.0.0.1", port: Int = 11435, apiKey: String? = nil) {
        self.host = host
        self.port = port
        self.apiKey = apiKey
    ...
    /// Force-refresh the preset bundle from fusion.ai (via the server proxy
    ...
    // pipeline implemented by `fusion/admin/ms_downloader.py`. Task / model
    ...
    private func request<T: Decodable>(
        ...
        guard let url = components.url else { throw FusionClientError.invalidURL }
        ...
        guard let http = resp as? HTTPURLResponse else { throw FusionClientError.invalidResponse }

        if http.statusCode == 401, !isRetry {
            guard let key = apiKey, !key.isEmpty else {
                throw FusionClientError.unauthenticated
            }
        ...
        guard 200..<300 ~= http.statusCode else {
            ...
            throw FusionClientError.http(status: http.statusCode, body: bodyStr)
        }
