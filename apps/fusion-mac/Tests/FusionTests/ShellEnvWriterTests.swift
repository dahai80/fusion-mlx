...
final class ShellEnvWriterTests: XCTestCase {
    ...
    func testEnsureCLIShimWritesExecutableWrapperWithoutEditingShellFiles() throws {
        ...
        XCTAssertTrue(shimText.contains("Contents/MacOS/fusion-cli"))
    ...
    func testEnsureCLIShimDoesNotOverwriteExistingPublicCommand() throws {
        ...
        ShellEnvWriter.publicBinDirsOverrideForTests = [publicBin]
        setenv("PATH", "\(publicBin.path):/usr/bin", 1)
        let existing = publicBin.appendingPathComponent("fusion")
    ...
    func testShimExportsBootstrapBasePath() throws {
        ...
        try """
        #!/bin/sh
        printf "%s" "$Fusion_BASE_PATH" > \(shellQuote(output.path))
        ...
        let support = tempHome
            ...
            .appendingPathComponent("FusionMLX", isDirectory: true)
        try FileManager.default.createDirectory(at: support, withIntermediateDirectories: true)
        try "/tmp/custom-fusion\n".write(
        ...
        XCTAssertEqual(try String(contentsOf: output, encoding: .utf8), "/tmp/custom-fusion")
    }
    ...
    private func makeFakeAppURL() throws -> URL {
        let appURL = tempHome
            ...
            .appendingPathComponent("FusionMLX.app", isDirectory: true)
        let cli = appURL
            .appendingPathComponent("Contents", isDirectory: true)
            .appendingPathComponent("MacOS", isDirectory: true)
            .appendingPathComponent("fusion-cli")
