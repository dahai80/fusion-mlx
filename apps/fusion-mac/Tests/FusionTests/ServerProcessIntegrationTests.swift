...
//   TEST_RUNNER_Fusion_INTEGRATION=1 \
//   TEST_RUNNER_Fusion_PYTHON_OVERRIDE="$PYBIN" \
//   TEST_RUNNER_Fusion_DEV_SERVER_SCRIPT="$REPO/apps/fusion-mac/Scripts/dev_server.py" \
//     xcodebuild -project apps/fusion-mac/FusionMLX.xcodeproj \
//                -scheme FusionMLX \
//                -only-testing:FusionMLXTests/ServerProcessIntegrationTests \
//                test
...
final class ServerProcessIntegrationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        guard ProcessInfo.processInfo.environment["Fusion_INTEGRATION"] == "1" else {
            throw XCTSkip("Set Fusion_INTEGRATION=1 to run integration smoke tests.")
        }
    }

    func testSpawnAndCleanShutdown() async throws {
        // The dev override pair must both be set — otherwise we'd need the
        // bundled venvstacks framework to satisfy `python -m fusion.cli`. Skip
        // with an actionable message rather than throw an opaque spawn
        // failure halfway through.
        let env = ProcessInfo.processInfo.environment
        guard let pythonOverride = env["Fusion_PYTHON_OVERRIDE"], !pythonOverride.isEmpty,
              FileManager.default.isExecutableFile(atPath: pythonOverride),
              let devScript = env["Fusion_DEV_SERVER_SCRIPT"], !devScript.isEmpty,
              FileManager.default.fileExists(atPath: devScript)
        else {
            throw XCTSkip(
                "Integration smoke test needs Fusion_PYTHON_OVERRIDE + " +
                "Fusion_DEV_SERVER_SCRIPT set. See file header for the command."
            )
        }
        ...
        let tempBase = FileManager.default.temporaryDirectory
            .appendingPathComponent("FusionMLX-integ-\(UUID().uuidString)", isDirectory: true)
        ...
        case .portConflict(let conflict):
            XCTFail("Port \(port) reported in-use before spawn (isFusion=\(conflict.isFusion)).")
            return
        }
