...
final class ReleasesCheckerTests: XCTestCase {
    ...
    func testFindMatchingDMGSupportsMacOSRangeAssets() {
        let sequoia = "FusionMLX-0.4.4-macos15-sequoia.dmg"
        let tahoeAndNext = "FusionMLX-0.4.4-macos26-27.dmg"
        let assets = [
            asset(sequoia),
            asset(tahoeAndNext),
        ]
    ...
    func testFindMatchingDMGPrefersExactAssetOverRangeAsset() {
        let range = "FusionMLX-0.4.4-macos26-27.dmg"
        let exact = "FusionMLX-0.4.4-macos27-beta.dmg"
        let assets = [
            asset(range),
            asset(exact),
        ]
...
final class UpdateControllerPrefsTests: XCTestCase {
    ...
    func testLegacyAutoDownloadPrefMigratesToAutoNotify() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fusion-update-prefs-\(UUID().uuidString)")
