// Round-trip JSON fixtures captured from a live FusionMLX server through each
...
final class DTOFixtureTests: XCTestCase {
    ...
    // Matches the JSONDecoder config in FusionClient: snake_case keys → camelCase
    ...
    func testStatsSessionFixtureDecodes() throws {
        ...
        XCTAssertNotNil(stats.cliPrefix,
                        "Stats must carry cli_prefix so Integrations can render `fusion launch …` commands.")
