#!/usr/bin/env bash
# build.sh — produce a runnable FusionMLX.app for local manual testing.
#
# Side-by-side Swift bundle path: this builds `FusionMLX.app` alongside the
# legacy Python/PyObjC `FusionMLX.app` until the Swift app becomes the primary
# release channel.
# Pipeline:
...
#   4. embed the fusion package from the worktree and ad-hoc sign
#
...
#   1. $FUSION_DONOR_APP — explicit override (e.g. /Applications/FusionMLX.app).
...
#   3. /Applications/FusionMLX.app — last-resort fallback when --no-rebuild-donor
#                               is set and no local export exists.
#
# Usage:
#   apps/fusion-mac/Scripts/build.sh                    # Release, auto-rebuild donor when stale
#   apps/fusion-mac/Scripts/build.sh debug              # Debug build instead
#   apps/fusion-mac/Scripts/build.sh swift              # rebuild Swift app only; reuse existing _export/
#   apps/fusion-mac/Scripts/build.sh swift debug        # Debug Swift app rebuild; reuse existing _export/
#   apps/fusion-mac/Scripts/build.sh swift-fast         # like swift, but skip embedded native signing
#   apps/fusion-mac/Scripts/build.sh release --bare     # skip Python embed
#                                                       (no server, just the
#                                                       AppView shell)
#   apps/fusion-mac/Scripts/build.sh release --with-custom-kernel
#                                                     # build and bundle optional
#                                                     # native custom kernels
#   apps/fusion-mac/Scripts/build.sh --rebuild-donor    # force venvstacks rebuild
#   apps/fusion-mac/Scripts/build.sh --no-rebuild-donor # never rebuild; use
#                                                       existing donor even if stale
#
# Env overrides:
#   FUSION_DONOR_APP=/path/to/FusionMLX.app    # explicit donor (bypasses venvstacks)
#   FUSION_EXPORT_DIR=/path/to/_export    # override the venvstacks export tree
#                                       (release builds set this per macOS target)
#   FUSION_NEXT_OUT=/path/to/output_dir   # final stage location
#   PYTHON_BIN=/path/to/python3         # python used for venvstacks driver
#                                       (default: PATH lookup of python3)
#   FUSION_WITH_CUSTOM_KERNEL=1           # same as --with-custom-kernel
#   FUSION_CUSTOM_KERNEL_DEPLOYMENT_TARGET=15.0
...
BARE=0
WITH_CUSTOM_KERNEL="${FUSION_WITH_CUSTOM_KERNEL:-0}"
...
CUSTOM_KERNEL_DIR="$REPO_ROOT/fusion_mlx/custom_kernels/glm_moe_dsa"
# FUSION_EXPORT_DIR overrides the venvstacks export tree we copy Python
...
LOCAL_EXPORT="${FUSION_EXPORT_DIR:-$PACKAGING_DIR/_export}"

# FUSION_DONOR_APP is "explicit" only when the user set it; the default
# (/Applications/FusionMLX.app) is treated as a fallback, not an override.
FUSION_DONOR_APP_SET="${FUSION_DONOR_APP+1}"
FUSION_DONOR_APP="${FUSION_DONOR_APP:-/Applications/FusionMLX.app}"
OUTPUT_DIR="${FUSION_NEXT_OUT:-$PROJECT_DIR/build/Stage}"
...
_custom_kernel_deployment_target() {
    printf "%s\n" "${FUSION_CUSTOM_KERNEL_DEPLOYMENT_TARGET:-${MACOSX_DEPLOYMENT_TARGET:-15.0}}"
}
...
_clean_custom_kernel_build_artifacts() {
    ...
    if [ -d "$REPO_ROOT/build" ]; then
        find "$REPO_ROOT/build" \
            -type d \
            -name "fusion.custom_kernels.glm_moe_dsa._ext" \
            -prune \
            -exec rm -rf {} +
    fi
}
...
_build_custom_kernels() {
    ...
    (
        ...
        export FUSION_CUSTOM_KERNEL_DEPLOYMENT_TARGET="$deployment_target"
    ...
    [ -f "$CUSTOM_KERNEL_DIR/fusion_glm_kernels.metallib" ] \
...
resolve_donor_layers() {
    ...
    # Explicit FUSION_DONOR_APP override → use it, skip rebuild logic entirely.
    if [ -n "$FUSION_DONOR_APP_SET" ]; then
        [ -d "$FUSION_DONOR_APP" ] || die "FUSION_DONOR_APP set but not found: $FUSION_DONOR_APP"
        DONOR_LAYERS="$(_app_python_layers_path "$FUSION_DONOR_APP")" \
            || die "FUSION_DONOR_APP missing bundled Python layers: $FUSION_DONOR_APP"
        DONOR_SOURCE="FUSION_DONOR_APP=$FUSION_DONOR_APP"
        return
    fi
    ...
    case "$REBUILD_DONOR" in
        ...
        never)
            ...
            elif [ -d "$FUSION_DONOR_APP" ]; then
                DONOR_LAYERS="$(_app_python_layers_path "$FUSION_DONOR_APP")" \
                    || die "Fallback donor missing bundled Python layers: $FUSION_DONOR_APP"
                DONOR_SOURCE="$FUSION_DONOR_APP (fallback, --no-rebuild-donor)"
            else
                die "No donor available: $LOCAL_EXPORT and $FUSION_DONOR_APP both missing, --no-rebuild-donor prevents rebuild."
            fi
            ;;
        auto)
...
# --- Derive bundle version from fusion_mlx/_version.py --------------------------
#
# fusion_mlx/_version.py is the canonical source — pyproject.toml reads it via
...
log "Bundle version: $APP_VERSION (build $BUILD_NUMBER) — from fusion_mlx/_version.py"

# --- xcodebuild -----------------------------------------------------------

log "Building FusionMLX ($CONFIG)…"
mkdir -p "$BUILD_DIR"

log "Resolving Swift package dependencies…"
xcodebuild -resolvePackageDependencies \
    -project "$PROJECT_DIR/FusionMLX.xcodeproj" \
    -scheme FusionMLX \
...
XCODE_APP="$BUILD_DIR/Build/Products/$CONFIG/FusionMLX.app"
...
mkdir -p "$OUTPUT_DIR"
STAGED_APP="$OUTPUT_DIR/FusionMLX.app"
...
# --- Embed fusion package ---------------------------------------------------

if [ "$WITH_CUSTOM_KERNEL" = "1" ]; then
    _build_custom_kernels
fi

log "Copying fusion package from source tree…"
rm -rf "$RESOURCES_DIR/fusion_mlx"
mkdir -p "$RESOURCES_DIR/fusion_mlx"
...
CLI_WRAPPER="$STAGED_APP/Contents/MacOS/fusion-cli"
cat > "$CLI_WRAPPER" <<'EOF'
#!/bin/sh
set -eu
...
ICON_BUNDLE="$REPO_ROOT/apps/fusion-mac/Resources/AppIcon.icon"
XCASSETS_DIR="$REPO_ROOT/apps/fusion-mac/Resources/Assets.xcassets"
...
echo "  ~/Library/Application Support/Fusion-MLX/logs/server.log"
