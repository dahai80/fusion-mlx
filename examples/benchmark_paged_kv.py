...
logger = logging.getLogger("benchmark_paged_kv")
...
def run_config(repo, prompt_ids, max_tokens, warmup, repeats, paged=False):
    import mlx_lm
    ...
    if paged:
        settings = ModelSettings(
            ...
            fusion_paged_kv_block_size=16,
            fusion_paged_kv_num_blocks=8192,
        )
    else:
...
def main():
    ...
    for key in args.models:
        ...
        prompt_ids = tok.encode(PROMPT_LONG)
        for paged, label in [
            (False, "default_kv"),
            (True, "paged_kv"),
        ]:
            ...
            try:
                ...
                rows.append({"model": key, "config": label, "error": str(e)[:80]})
                continue
            row = {
                "model": key,
                "config": label,
    ...
    print("paged_kv benchmark complete, rows:", len(rows))
...
def _write_report(args, rows):
    ...
    path = out_dir / f"perf-paged-kv-{ts}.md"
    lines = []
    lines.append("# Paged-KV Performance Comparison")
    lines.append("")
    lines.append(f"- Date: {ts}")
    lines.append("- Hardware: M5 Max 128GB (40-core GPU)")
    lines.append("- fusion-mlx branch feat/enhance-arch-0826; Phase 1 Paged-KV")
    lines.append(
        f"- max_tokens={args.max_tokens} warmup={args.warmup} repeats={args.repeats} median-of-N"
    )
    lines.append(f"- prompt=long ({len(PROMPT_LONG)} chars), greedy deterministic")
    lines.append(
        "- configs: default_kv=upstream KVCache | paged_kv=FusionPagedKVCache (block_size=16, Python block mgmt, raw KV)"
    )
    lines.append("")
    lines.append("## Honest expectation")
    lines.append("")
    lines.append(
        "Single-stream decode is expected to be **neutral** (within noise). "
        "Paged-KV's win is concurrency and fragmentation reduction, which "
        "materializes in Phase 3 continuous batching. This Phase 1 benchmark "
        "verifies correctness (bit-exact tokens) and measures single-stream "
        "overhead of the Python block-management layer."
    )
    lines.append("")
    ok = [r for r in rows if "error" not in r]
    if ok:
        lines.append(
            "| model | config | prompt_tok | gen_tok | prefill tok/s | decode tok/s | wall s | peak mem GB | post used GB |"
        )
        lines.append("|---|---|---|---|---|---|---|---|---|")
        for r in ok:
            lines.append(
                ...
                f"| {r['peak_mem_gb']} | {r['post_used_gb']} |"
            )
    by_model = {}
    for r in ok:
    ...
    if by_model:
        lines.append("")
        lines.append("## Paged-KV vs default (single-stream)")
        lines.append("")
        lines.append(
            "| model | default decode | paged decode | delta% | default prefill | paged prefill | delta% |"
        )
        lines.append("|---|---|---|---|---|---|---|")
        for m, cfgs in by_model.items():
            dft = cfgs.get("default_kv")
            pg = cfgs.get("paged_kv")
            if dft and pg:
                d_dec = (
                    round(
                        (pg["decode_tps"] - dft["decode_tps"])
                        / dft["decode_tps"]
                        * 100,
                        2,
                    )
                ...
                d_pre = (
                    round(
                        (pg["prefill_tps"] - dft["prefill_tps"])
                        / dft["prefill_tps"]
                        * 100,
                        2,
                    )
                ...
                lines.append(
                    f"| {m} | {dft['decode_tps']} | {pg['decode_tps']} | {d_dec}% | {dft['prefill_tps']} | {pg['prefill_tps']} | {d_pre}% |"
                )
    ...
    lines.append("")
    lines.append(
        "Paged-KV is bit-exact with upstream KVCache (verified by unit test "
        "and real-model integration test on qwen3-0.6b: identical token "
        "sequence for greedy decode). The cache stores raw (unquantized) KV "
        "per block; quantize-on-fetch is a Phase 2 GEMV concern."
    )
    path.write_text("\n".join(lines))
    print("report:", path)
