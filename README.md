...
- **Distributed pipeline parallelism** - split a transformer forward at a layer boundary across nodes; `/distributed/load_shard`, `/distributed/pipeline_step`, `/distributed/sync_weights` endpoints with bit-exact activation serialization (base64 `.npy`) and path-traversal confinement. See [docs/distributed-pipeline.md](docs/distributed-pipeline.md)
