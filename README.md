...
| `FUSION_CLOUD_MODEL` | _(unset)_ | #811. Cloud-fallback model id passed to `CloudRouter` when `--cloud-router` is enabled but `--cloud-model` is not supplied on the CLI. Required at construction; if neither the CLI flag nor this env var is set, the server fails visibly with guidance instead of crashing on a `TypeError`. |
| `FUSION_VIDEO_GEN_TIMEOUT` | `7200` (2h) | #811. Per-request deadline (seconds) for a single video generation. A generation that exceeds it is treated as a hung pipeline: the worker thread cannot be cancelled from Python, so the video executor is **poisoned** — later video requests fast-fail with a "restart required" error and a fresh worker is spawned (it reloads model weights on its own thread-local stream). Raise it for very long jobs (SkyReels-V3 R2V 14B 720p 30-step ≈ 1h); lower it to catch hangs sooner at the cost of killing legitimately long jobs. |
| `FUSION_FORCE_INLINE_PREFILL` | `false` | #811. Escape hatch that forces the scheduler to run prefill inline (single forward) instead of chunked prefill. Chunked prefill normally auto-falls back to inline on failure (#811 R-21); set this to `1`/`true` to skip chunked prefill entirely — useful for a model/shape whose chunked path is known-broken or for isolating a prefill OOM to the chunking logic. Off by default; chunked prefill is the production path. |
