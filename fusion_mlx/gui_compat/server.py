...
from fusion_mlx.gui_compat.mlx_integration import GenerationConfig
from fusion_mlx.gui_compat.model_manager import (
    get_model_manager,
    shutdown_model_manager,
)
from fusion_mlx.gui_compat.models import AppSettings, Model
...
class ChatMessage(BaseModel):
    role: str | None = None
    content: str | list[ChatMessageContent] | None = None
    image: str | None = None
            ...
            for p in content:
                if p.type == "text" and p.text:
                    ...
                    if isinstance(p.image_url, dict):
                        u = (
                            p.image_url.get("url", "")
                            or p.image_url.get("image", "")
                            or p.image_url.get("data", "")
                        )
            ...
            if u.startswith("data:image/"):
                ...
                ext = (
                    ...
                    if "png" in hdr
                        ...
                        ".gif" if "gif" in hdr else ".webp" if "webp" in hdr else ".jpg"
                    )
                )
                ...
                ext = (
                    ".jpg"
                    if bd.startswith(b"\xff\xd8\xff")
                    else (
                        ".png"
                        if bd.startswith(b"\x89PNG")
                        else (
                            ".gif"
...
def _model_dict(m):
    return {
        ...
        "size_gb": m.size_gb,
        "estimated_memory_gb": m.estimated_memory_gb,
        "mlx_compatible": m.mlx_compatible,
        "has_mlx_version": m.has_mlx_version,
        "mlx_repo_id": m.mlx_repo_id,
        "tags": m.tags,
...
def get_gui_compat_router() -> APIRouter:
            ...
            "models": [
                {
                    ...
                    "author": (
                        m.huggingface_id.split("/")[0]
                        if m.huggingface_id and "/" in m.huggingface_id
                        else "unknown"
                    ),
                }
    ...
    async def load_model(
        ...
        try:
            ...
            can, msg = get_system_monitor().check_model_compatibility(
                mr.memory_required_gb
            )
            if not can and "MLX requires Apple Silicon" in msg:
                raise HTTPException(status_code=400, detail=msg)
            ok = await mm.load_model_async(
                model_name=model_name, model_path=mr.path, priority=priority
            )
            resp = {"message": f"Model '{model_name}' loaded", "status": "loaded"}
            if "warning" in msg.lower():
                resp["memory_warning"] = msg
            if not ok:
                raise HTTPException(
                    status_code=500, detail=f"Failed to load '{model_name}'"
                )
            return resp
            ...
            if remove_files and mr.path:
                try:
                    if ".cache" in mr.path and "models--" in mr.path:
                        ...
                        (
                            shutil.rmtree(mr.path)
                            if os.path.isdir(mr.path)
                            else os.remove(mr.path)
                        )
                except Exception as fe:
                    logger.warning(f"File remove failed: {fe}")
            return {
    ...
    async def gen_text(
        ...
        try:
            ...
            cfg = GenerationConfig(
                max_tokens=rd.get("max_tokens", 100),
                temperature=rd.get("temperature", 0.0),
                top_p=rd.get("top_p", 1.0),
                top_k=rd.get("top_k", 0),
                repetition_penalty=rd.get("repetition_penalty", 1.0),
                repetition_context_size=rd.get("repetition_context_size", 20),
                seed=rd.get("seed"),
            )
            ...
            return {
                "model": model_name,
                "prompt": r.prompt,
                "text": _parse_harmony_response(r.text),
                "usage": {
    ...
    async def sys_status():
        ...
        return {
            "status": "running",
            "system": ss,
            "model_manager": get_model_manager().get_system_status(),
            "mlx_compatible": ss["mlx_compatible"],
        }
    ...
    async def disc_models(query: str = "", limit: int = 20, sort: str = "downloads"):
        try:
            ms = get_huggingface_client().search_mlx_models(
    ...
    async def disc_emb(query: str = "", limit: int = 20):
        try:
            ms = get_huggingface_client().search_embedding_models(
    ...
    async def disc_compat(query: str = "", max_memory_gb: float | None = None):
        try:
            if max_memory_gb is None:
                max_memory_gb = get_system_monitor().get_memory_info().total_gb * 0.8
            ms = get_huggingface_client().search_compatible_models(query, max_memory_gb)
            return {
                "models": [
                    {
                        **_model_dict(m),
                        "memory_fit": f"{m.estimated_memory_gb:.1f}GB required, {max_memory_gb:.1f}GB available",
                    }
                    for m in ms
                ],
    ...
    async def disc_model_detail(model_id: str):
        try:
            ...
            can, msg = get_system_monitor().check_model_compatibility(
                m.estimated_memory_gb or 0
            )
            return {
                **_model_dict(m),
                "library_name": m.library_name,
                "pipeline_tag": m.pipeline_tag,
                "system_compatible": can,
    ...
    async def install_model(
        ...
        try:
            ...
            if not mi:
                raise HTTPException(
                    status_code=404,
                    detail=f"Model '{req.model_id}' not found on HuggingFace",
                )
            if not mi.mlx_compatible:
                raise HTTPException(
                    status_code=400, detail=f"Model '{req.model_id}' not MLX compatible"
                )
            ...
            if db.query(Model).filter(Model.name == name).first():
                return {
                    "message": f"Model '{name}' already installed",
            ...
            nm = Model(
                ...
                huggingface_id=req.model_id,
                memory_required_gb=int(em),
                status="unloaded",
            )
            nm.set_metadata(
                {
                    ...
                    "tags": mi.tags,
                    "size_gb": mi.size_gb,
                    "estimated_memory_gb": mi.estimated_memory_gb,
                }
            )
            db.add(nm)
            db.commit()
    ...
    async def mgr_status():
        ...
        return {
            ...
            "system_status": mm.get_system_status(),
    ...
    async def admin_ui():
        ...
        return HTMLResponse(
            content=tp.read_text(encoding="utf-8").replace("{{ version }}", __version__)
        )
    ...
    async def dbg_model(model_id: str):
        try:
            ...
            for pat in [
                ...
                if ms:
                    dbg["params_billion"] = float(ms[0])
                    break
            bits = (
                4
                if "4bit" in mn or "4-bit" in mn
                else (8 if "8bit" in mn or "8-bit" in mn else 16)
            )
            dbg["quant_bits"] = bits
            return dbg
        except Exception as e:
...
def create_gui_router() -> FastAPI:
    ...
    @app.get("/")
    async def _root():
        return {"name": "fusion_gui API", "version": __version__, "status": "running"}
