...
class HuggingFaceClient:
    ...
    def __init__(self, token: str | None = None):
        ...
        self.token = token
        self.mlx_tags = {"mlx", "mlx-lm", "apple-silicon", "metal", "quantized"}
    ...
    def search_mlx_models(
        self, query: str = "", limit: int = 50, sort: str = "downloads"
    ) -> list[ModelInfo]:
        """
        ...
        try:
            ...
            models = sorted(
                models, key=lambda x: getattr(x, "downloads", 0), reverse=True
            )[:limit]
    ...
    def search_models_by_author(self, author: str, limit: int = 20) -> list[ModelInfo]:
        ...
        try:
            models = list_models(
                author=author, limit=limit, sort="downloads", direction=-1
            )
    ...
    def _extract_model_info(self, model) -> ModelInfo | None:
        ...
        try:
            ...
            mlx_compatible = (
                any(
                    tag and tag.lower() in self.mlx_tags
                    for tag in tags
                    if tag is not None
                )
                or library_name == "mlx"
                or (model.id and "mlx-community/" in model.id)
                or "mlx" in tags
            )
    ...
    def _estimate_model_size(self, model) -> float | None:
        """Calculate model memory requirements from parameter count and quantization."""
        try:
            # Check if model.id is None
            if not hasattr(model, "id") or model.id is None:
                logger.warning(
                    "Model object has no id attribute or id is None in _estimate_model_size"
                )
                return None
            ...
            try:
                ...
                for pattern in param_patterns:
                    ...
                    if matches:
                        ...
                        logger.debug(
                            f"Found parameter count in model card for {model.id}: {param_count_billions}B"
                        )
                        break
            ...
            if not param_count_billions:
                try:
                    ...
                    if (
                        hasattr(model_info_data, "safetensors")
                        and model_info_data.safetensors
                    ):
                            ...
                            if isinstance(metadata, dict):
                                ...
                                if "total" in metadata:
                                    ...
                                    if (
                                        ...
                                        matched_pattern = "safetensors-total"
                                        logger.debug(
                                            f"Found parameter count in safetensors for {model.id}: {param_count_billions}B"
                                        )
                                        break
                                ...
                                for pattern in param_patterns:
                                    matches = re.findall(
                                        pattern, metadata_str, re.IGNORECASE
                                    )
                                    if matches:
                                        param_count_billions = float(matches[0])
                                        matched_pattern = f"safetensors-{pattern}"
                                        logger.debug(
                                            f"Found parameter count in safetensors metadata for {model.id}: {param_count_billions}B"
                                        )
                                        break
                    ...
                    logger.debug(
                        f"Could not parse safetensors metadata for {model.id}: {e}"
                    )
            ...
            if param_count_billions:
                ...
                if (
                    "4bit" in model_name
                    or "4-bit" in model_name
                    or "qat-4bit" in model_name
                    or "dwq" in model_name
                ):
                    bits_per_param = 4
                    ...
                    "8bit" in model_name
                    or "8-bit" in model_name
                    or "int8" in model_name
                ):
                    bits_per_param = 8
                ...
                base_memory_gb = (param_count_billions * 1e9 * bits_per_param) / (
                    8 * 1024**3
                )
                ...
                logger.debug(
                    f"Model {model.id}: {param_count_billions}B params (source: {matched_pattern}), {bits_per_param}-bit = {total_memory_gb:.1f}GB"
                )
                return total_memory_gb

            # If we can't determine parameter count, don't guess - return None
            logger.debug(
                f"Could not determine parameter count for {model.id} - no size estimate available"
            )
            return None
    ...
    def _determine_model_type(self, model, tags: list[str]) -> str:
        ...
        has_vision = any(
            tag in tags_lower
            for tag in ["vision", "image", "visual", "computer-vision"]
        )
        has_audio = any(
            tag in tags_lower
            for tag in ["audio", "speech", "automatic-speech-recognition"]
        )
        has_text = any(
            tag in tags_lower
            for tag in ["text-generation", "language-modeling", "causal-lm"]
        )
        ...
        if has_audio or any(
            tag in tags_lower for tag in ["text-to-speech", "tts", "whisper"]
        ):
            return "audio"
    ...
    def get_specific_embedding_models(self) -> list[str]:
        ...
        try:
            ...
            if dynamic_models:
                ...
                combined = dynamic_ids + [
                    model for model in static_models if model not in dynamic_ids
                ]
    ...
    def get_model_categories(self) -> dict[str, list[str]]:
        ...
        try:
            ...
            for model in popular_models:
                ...
                if any(
                    tag in (getattr(model, "tags", None) or [])
                ...
                if model.model_type in ["text", "multimodal"] and any(
                    tag in (getattr(model, "tags", None) or [])
                    for tag in ["chat", "conversational", "instruct", "assistant"]
                ):
                ...
                if (
                    ...
                    and not any(model.id in cat for cat in categories.values())
                    and model.id not in categories["Popular Chat"]
                ):
    ...
    def search_compatible_models(
        self, query: str, max_memory_gb: float
    ) -> list[ModelInfo]:
        ...
        for audio_query in audio_queries:
            try:
                ...
                for model in models:
                    ...
                    # Safely handle case where model.id might be None
                    model_id_check = model.id and any(
                        keyword in model.id.lower()
                        for keyword in ["whisper", "tts", "speech", "audio", "parakeet"]
                    )
                    if (
                ...
                logger.warning(
                    f"Error searching for audio models with query '{audio_query}': {e}"
                )
                continue
        ...
        for tts_query in tts_queries:
            try:
                ...
                for model in models:
                    ...
                    if model_id_check or any(
                        tag in (getattr(model, "tags", None) or [])
                        for tag in ["text-to-speech", "tts"]
                    ):
                        if model.id not in [m.id for m in tts_models]:
                            tts_models.append(model)
            except Exception as e:
                logger.warning(
                    f"Error searching TTS models with query '{tts_query}': {e}"
                )
                continue
    ...
    def search_stt_models(self, query: str = "", limit: int = 10) -> list[ModelInfo]:
        ...
        try:
            ...
            try:
                logger.info(
                    "Searching for STT models using pipeline_tag=automatic-speech-recognition&library=mlx"
                )
                ...
                logger.info(
                    f"Found {len(pipeline_models_list)} MLX STT models with library filter"
                )
                ...
                for model in pipeline_models_list:
                    try:
                        ...
                        logger.warning(
                            f"Error processing STT model {getattr(model, 'id', 'unknown')}: {e}"
                        )
                        continue
            ...
            for stt_query in stt_queries:
                try:
                    ...
                    logger.warning(
                        f"Error searching STT models with query '{stt_query}': {e}"
                    )
                    continue
    ...
    def search_embedding_models(
        ...
        try:
            ...
            try:
                logger.info(
                    "Searching for embedding models using pipeline_tag=feature-extraction&library=mlx"
                )
                ...
                logger.info(
                    f"Found {len(feature_models_list)} MLX feature-extraction models"
                )
            ...
            try:
                logger.info(
                    "Searching for embedding models using pipeline_tag=sentence-similarity&library=mlx"
                )
                ...
                logger.info(
                    f"Found {len(sentence_models_list)} MLX sentence-similarity models"
                )
            ...
            logger.info(
                f"Total MLX embedding models from pipeline searches: {total_pipeline_models}"
            )

            # Method 1.5: Fallback without library filter for broader coverage
            try:
                logger.info(
                    "Fallback: Searching feature-extraction models without library filter"
                )
                ...
                logger.info(
                    f"Found {len(all_embedding_list)} total feature-extraction models"
                )
                ...
                for model in all_embedding_list:
                    if model.id not in models_dict:  # Only add if not already found
                        ...
                        # Safely handle case where model.id might be None
                        model_id_check = model.id and (
                            model.id.startswith("mlx-community/")
                            or "mlx" in model.id.lower()
                        )
                        is_mlx = (
                            any(tag.lower() in self.mlx_tags for tag in model_tags)
                            or model_id_check
                        )
                        ...
                        if is_mlx:
                            ...
                            logger.debug(
                                f"Found additional MLX embedding model: {model.id}"
                            )

                logger.info(
                    f"Added {mlx_fallback_count} additional MLX embedding models from fallback search"
                )
            ...
            try:
                ...
                for embedding_query in embedding_queries:
                    try:
                        ...
                        logger.info(
                            f"Found {len(tag_models_list)} models with MLX tag and '{embedding_query}' search"
                        )

                        for model in tag_models_list:
                            models_dict[model.id] = model
                            logger.debug(
                                f"Found embedding model via tag search: {model.id}"
                            )

                    except Exception as e:
                        logger.warning(
                            f"Error searching with query '{embedding_query}': {e}"
                        )
                        continue
            ...
            try:
                ...
                for pattern in embedding_patterns:
                    try:
                        ...
                        logger.info(
                            f"Found {len(pattern_models_list)} models with pattern '{pattern}'"
                        )
                        ...
                        for model in pattern_models_list:
                            ...
                            if is_mlx:
                                models_dict[model.id] = model
                                logger.debug(
                                    f"Found embedding model via pattern search: {model.id}"
                                )
            ...
            logger.info(
                f"Returning {min(len(embedding_models), limit)} embedding models"
            )
    ...
    def search_vision_models(self, query: str = "", limit: int = 10) -> list[ModelInfo]:
        ...
        try:
            ...
            try:
                logger.info(
                    "Searching for vision models using HuggingFace pipeline_tag=image-text-to-text, library=mlx"
                )
                ...
                pipeline_models_list = list(pipeline_models)
                logger.info(
                    f"Found {len(pipeline_models_list)} image-text-to-text models with MLX library"
                )
            ...
            try:
                logger.info(
                    "Searching for image-text-to-text models without library filter"
                )
                ...
                logger.info(
                    f"Found {len(all_vision_models_list)} total image-text-to-text models"
                )
                ...
                logger.info(
                    f"Found {mlx_count} MLX-compatible vision models from broader search"
                )
            ...
            try:
                ...
                tag_models_list = list(tag_models)
                logger.info(
                    f"Found {len(tag_models_list)} models with MLX tag and 'llava' search"
                )
                ...
                for model in tag_models_list:
                    ...
                    model_id_check = model.id and any(
                        keyword in model.id.lower()
                        for keyword in ["llava", "vision", "vl", "multimodal"]
                    )
                    if (
                        hasattr(model, "pipeline_tag")
                        and model.pipeline_tag == "image-text-to-text"
                    ) or model_id_check:
                        if model.id not in models_dict:  # Avoid duplicates
                            models_dict[model.id] = model
                            vision_count += 1
                            logger.debug(
                                f"Found vision model via tag search: {model.id}"
                            )

                logger.info(
                    f"Added {vision_count} additional vision models from tag search"
                )
            ...
            for model in models:
                try:
                    ...
                    if info and info.mlx_compatible:
                        ...
                        logger.debug(
                            f"Added vision model: {info.id} (type: {info.model_type})"
                        )
                except Exception as e:
...
def get_huggingface_client(token: str | None = None) -> HuggingFaceClient:
