...
class MLXTrayApp(rumps.App):
    ...
    def __init__(self, port: int = 8000, host: str = "127.0.0.1"):
        ...
        self.admin_item = rumps.MenuItem(
            "Open Admin Interface", callback=self.open_admin
        )
        self.unload_item = rumps.MenuItem(
            "Unload All Models", callback=self.unload_all_models
        )
        ...
        # GitHub link
        self.github_item = rumps.MenuItem(
            "📂 View on GitHub", callback=self.open_github
        )
        ...
        try:
            db_manager = get_database_manager()
            bind_to_all = db_manager.get_setting("bind_to_all_interfaces", False)
            self.bind_checkbox = rumps.MenuItem(
                "Bind to All Interfaces", callback=self.toggle_bind_interfaces
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if queue_size > 0:
                    ...
                    self.models_item.title = (
                        f"Models: {self.loaded_models_count} loaded"
                    )
                ...
                self.memory_usage = (
                    f"{total_model_memory:.1f}GB / {total_system_memory:.1f}GB"
                )
                memory_percent = model_manager.get("memory_usage_percent", 0)
                self.memory_item.title = (
                    f"Memory: {self.memory_usage} ({memory_percent:.1f}%)"
                )
                ...
                self.unload_item.set_callback(
                    self.unload_all_models if self.loaded_models_count > 0 else None
                )
        ...
        if response == 1:  # OK clicked
            try:
                ...
                models_response = requests.get(
                    f"{self.base_url}/v1/manager/status", timeout=10
                )
                if models_response.status_code == 200:
                    ...
                    for model_name in loaded_models:
                        try:
                            ...
                            if unload_response.status_code == 200:
                                ...
                                failed_count += 1
                                logger.error(
                                    f"Failed to unload {model_name}: {unload_response.status_code}"
                                )
                        except requests.exceptions.RequestException as e:
                            failed_count += 1
                    ...
                    raise requests.exceptions.RequestException(
                        f"HTTP {models_response.status_code}"
                    )

            except requests.exceptions.RequestException as e:
                logger.error(f"Failed to unload models: {e}")
                rumps.alert(
                    title="Error", message=f"Failed to unload models: {e}", ok="OK"
                )
    ...
    def toggle_bind_interfaces(self, sender):
        ...
        if response == 1:  # OK clicked
            try:
                ...
                rumps.alert(
                    title="Error", message=f"Failed to update setting: {e}", ok="OK"
                )
    ...
    def restart_server(self):
        ...
        self.server_thread = threading.Thread(
            target=self.start_server_background, name="mlx_server", daemon=True
        )
        self.server_thread.start()

        # Wait for server to be ready
        ready_thread = threading.Thread(
            target=self.wait_for_server, name="server_ready_check", daemon=True
        )
        ready_thread.start()
