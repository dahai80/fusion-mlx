...
class SystemMonitor:
    ...
    def get_gpu_memory_info(self) -> GPUMemory | None:
    ...
    def _get_apple_silicon_gpu_memory(self) -> GPUMemory | None:
    ...
    def _get_apple_chip_info(self) -> str | None:
    ...
    def check_model_compatibility(self, required_memory_gb: float) -> tuple[bool, str]:
        ...
        max_model_memory = (
            memory_info.total_gb * 0.8
        )  # 80% of total RAM available for models

        if required_memory_gb > max_model_memory:
            return (
                True,
                f"⚠️ Memory warning - This model requires {required_memory_gb:.1f}GB but only {max_model_memory:.1f}GB ({memory_info.total_gb:.1f}GB total * 80%) is typically available for models. Loading may cause system slowdown or instability.",
            )

        return (
            True,
            f"✅ Compatible - {required_memory_gb:.1f}GB required, {max_model_memory:.1f}GB available ({memory_info.total_gb:.1f}GB total * 80%)",
        )
    ...
    def log_system_info(self):
        ...
        if info.gpu_memory:
            logger.info(
                f"GPU: {info.gpu_memory.device_name}, {info.gpu_memory.total_gb:.1f}GB"
            )
