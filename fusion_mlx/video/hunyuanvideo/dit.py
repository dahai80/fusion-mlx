...
# HunyuanVideo DiT: 3D patchification, transformer blocks with
# full-attention + cross-attention for text conditioning.
...
HUNYUAN_VIDEO_CONFIG = {
    ...
    "in_channels": 16,
    "out_channels": 16,
    "text_embed_dim": 2048,  # CLIP-L(768) + CLIP-G(1280) = 2048
    "rope_dim": 128,
    "head_dim": 128,
}
...
class HunyuanAttention(nn.Module):
    def __init__(self, hidden_size, num_heads, head_dim=None, rope_dim=None):
        super().__init__()
        ...
        self.k_proj = nn.Linear(hidden_size, num_heads * self.head_dim, bias=True)
        self.v_proj = nn.Linear(hidden_size, num_heads * self.head_dim, bias=True)
        self.out_proj = nn.Linear(num_heads * self.head_dim, hidden_size, bias=True)
...
class HunyuanDiTBlock(nn.Module):
    def __init__(
        self, hidden_size, num_heads, mlp_ratio=4.0, head_dim=None, rope_dim=None
    ):
        super().__init__()
        self.adaln = HunyuanAdaLN(hidden_size)
        self.self_attn = HunyuanAttention(hidden_size, num_heads, head_dim, rope_dim)
        self.cross_attn = HunyuanAttention(hidden_size, num_heads, head_dim)
        self.mlp = HunyuanMLP(hidden_size, mlp_ratio)
    ...
    def __call__(self, x, text_emb, adaln_emb, rope=None):
        ...
        cross_out = self.cross_attn(
            self.adaln.norm(x) * (1 + scale_mlp) + shift_mlp, context=text_emb
        )
        x = x + cross_out
...
class HunyuanPatchEmbed3D(nn.Module):
    def __init__(self, in_channels=16, hidden_size=3072, patch_size=(1, 2, 2)):
        super().__init__()
...
class HunyuanVideoDiT(nn.Module):
    def __init__(self, config=None):
        ...
        self.rope_dim = cfg.get("rope_dim", self.head_dim)

        self.patch_embed = HunyuanPatchEmbed3D(
    ...
    def __call__(self, x, timestep, text_emb, image_cond=None):
        ...
        text_proj = self.caption_proj(text_emb)
    ...
    def from_pretrained(cls, model_path, config=None, **kwargs):
        ...
        for k, v in flat:
            if k in mapped:
                loaded[k] = mapped[k].astype(mx.float16)
            else:
                loaded[k] = v
