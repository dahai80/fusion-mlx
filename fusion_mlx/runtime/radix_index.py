...
``~/.cache/fusion-mlx/prefix_cache/<model>/`` as before — but it replaces the
...
class RadixStats:
    ...
    via ``rate(fusion_mlx_prefix_cache_radix_hits_total[5m])``.
    """
...
class RadixPrefixIndex:
    ...
    Lifecycle:
        ...
        idx.save("~/.cache/fusion-mlx/.../radix.index")
    ...
    *invalidation-safe* — a stale entry in the radix that no longer exists
    ...
    def save(self, path: str) -> None:
        ...
        other on-disk artefact in fusion-mlx uses. A partial write leaves
