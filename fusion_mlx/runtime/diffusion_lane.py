...
fresh ``pip install fusion-mlx`` lands a build that has the
...
def _break_mlx_vlm_eos_token_id_aliasing(model: Any, processor: Any) -> None:
    ...
    fusion-mlx's mlx-vlm floor moves past the fixed release this
...
class DiffusionEngine(BaseEngine):
    ...
    def _start_worker_once(self) -> None:
        ...
        with self._worker_start_lock:
            ...
            self._worker = threading.Thread(
                ...
                name="fusion-mlx-diffusion-worker",
                daemon=True,
            )
            self._worker.start()
    ...
    def build_prompt(
        ...
        if tools and self.supports_tool_calls:
            ...
            # is at least visible in ``fusion-mlx logs``.
            logger.warning(
        ...
        **kwargs,
        ...
        async with self._generation_lock:
            ...
            pump_thread = threading.Thread(
                target=pump,
                name="fusion-mlx-diffusion-pump",
                daemon=True,
            )
