...
- ``fusion_mlx.runtime.cache`` loads checkpoints during startup via
...
- ``fusion_mlx.runtime.disk_kv_checkpoint.get_stats`` returns a stats dict
...
# fusion_mlx.positioned_kv_cache now exists (#360 CLOSED) — provides
# positioned_update_and_fetch for pre-checkpoint writes (see lines 130,
# 443). NOTE: the scheduler hook that drives this checkpoint path
# (_maybe_disk_checkpoint / _safe_disk_checkpoint) is NOT yet wired into
# fusion_mlx.scheduler (see tests/unit/test_scheduler_disk_kv_hook.py),
# so there is no live runtime caller yet.
...
# fusion_mlx/positioned_kv_cache.py).
    ...
    *,
    ...
    Args:
        cache: ``list`` of MLX-LM cache layers (KVCache /
            ...
            using :func:`fusion_mlx.positioned_kv_cache.positioned_update_and_fetch`
...
def scan_checkpoints(root: str) -> list[tuple[str, float, int]]:
    ...
    - The startup loader hand-off in
      :mod:`fusion_mlx.runtime.cache` (a future iteration; today the loader
