...
# FUSION_MLX_MCP_CONFIG env var for explicit project-local configs.
#
# TODO: The vllm-mlx and rapid-mlx config paths below are from the source project
# (Rapid-MLX). Decide whether to keep these as back-compat fallbacks or replace
# them with fusion-mlx-specific paths.
CONFIG_SEARCH_PATHS = [
    "~/.config/fusion-mlx/mcp.json",
    "~/.config/fusion-mlx/mcp.yaml",
...
# Environment variable for config path. VLLM_MLX_MCP_CONFIG and
# RAPID_MLX_MCP_CONFIG are deprecated pre-rename aliases, still honored
# for back-compat.
CONFIG_ENV_VAR = "FUSION_MLX_MCP_CONFIG"
CONFIG_ENV_VAR_LEGACY = "RAPID_MLX_MCP_CONFIG"
...
def load_mcp_config(path: str | Path | None = None) -> MCPConfig:
    ...
    2. FUSION_MLX_MCP_CONFIG environment variable (or the deprecated
       RAPID_MLX_MCP_CONFIG / VLLM_MLX_MCP_CONFIG aliases)
    3. ~/.config/fusion-mlx/mcp.json or mcp.yaml (falling back to the
       pre-rename ~/.config/rapid-mlx/ or ~/.config/vllm-mlx/ locations)
...
def _find_config_file(
    ...
    # 2. Environment variable (with deprecated pre-rename aliases). Require a
    ...
    for env_var in (CONFIG_ENV_VAR, CONFIG_ENV_VAR_LEGACY, CONFIG_ENV_VAR_LEGACY2):
