...
"""RotatingKVCache subclass tailored for fusion-mlx's restored-cache contract.
...
the RHS slice when ``keys.shape[2] < l``, or (b) (when fusion-mlx zero-pads
...
always well-defined without any zero-padding trick on fusion-mlx's side.
"""
...
class PrefillReadyRotatingKVCache(RotatingKVCache):
    ...
    merge then either over-reads the RHS or, when fusion-mlx pre-pads with
