...
class ANEExecutionProvider:
    ...
    advertise ANE capability. The LLM autoregressive主干 (BatchedEngine /
    VLMBatchedEngine / ImageGenEngine) never inherits it, so the主干 stays
    Metal-only. All hooks default to "ANE disabled" so an engine that inherits
    but has no CoreML model behaves identically to a plain Metal engine.

    Red lines enforced here:
      - ane_pinned=True  : ANE engines are never LRU-evicted by the普通
        admission loop (权重常驻, eviction would lose compiled CoreML state).
