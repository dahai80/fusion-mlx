...
"""CLI entry point for ``fusion-mlx doctor`` — pure env-health probe.
...
``fusion-mlx bench --tier ...`` in PRs #1-#3 of the doctor refactor series;
...
def doctor_command(args: Any) -> None:
    ...
    ``args`` is the argparse namespace from ``fusion_mlx.cli``. We only read
    ``args.verbose`` (and reject the removed positional ``tier`` argument
    with a clear pointer to ``fusion-mlx bench --tier ...``).
    """
    # Hard removal: PRs #1-#3 deprecated the tier subcommands; this PR closes
    # the door. If a user still types ``fusion-mlx doctor smoke`` they hit
    # this branch and get pointed at the replacement.
    legacy_tier = getattr(args, "tier", None)
    if legacy_tier in _REMOVED_TIERS:
        print(
            f"fusion-mlx doctor {legacy_tier!s} was removed in 0.7.22.\n"
            f"Use:  fusion-mlx bench <model> --tier {legacy_tier}\n"
            "Doctor is now a pure environment-health check; "
            "model-validation tiers live in `fusion-mlx bench`.",
            file=sys.stderr,
        )
        sys.exit(2)
...
def render(report: Report, *, verbose: bool = False, stream=None) -> None:
    ...
    write("\n")
    write("┌" + "─" * 57 + "┐\n")
    write("│" + "\U0001fa7a Fusion-MLX Doctor".center(57) + "│\n")
    write("└" + "─" * 57 + "┘\n")
    write("\n")
