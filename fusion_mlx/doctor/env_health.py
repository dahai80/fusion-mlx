...
"""Environment-health probes for ``fusion-mlx doctor``.
...
optional dev tools. The user runs ``fusion-mlx doctor`` to answer one question
...
Total wall-clock for ``fusion-mlx doctor`` ≤ 5 s on a warm cache, dominated by
...
OPTIONAL_PACKAGES: list[tuple[str, str, str]] = [
    ("mlx-vlm", "mlx-vlm (vision extras)", "pip install 'fusion-mlx[vision]'"),
    ("mlx-audio", "mlx-audio (audio extras)", "pip install 'fusion-mlx[audio]'"),
    (
        "mlx-embeddings",
        "mlx-embeddings (embeddings extras)",
        "pip install 'fusion-mlx[embeddings]'",
    ),
]
...
def section_system() -> Section:
    ...
    ⚠ on:
      * non-Apple-Silicon (fusion-mlx targets M-series; works elsewhere but
        with Metal-fallback caveats).
      * < 20 GB free disk (model weights are big).
      * HF cache > 100 GB (suggest ``fusion-mlx rm`` cleanup).
    ...
    if not cache.exists():
        ...
        if cache_size_gb is None:
            ...
            s.add(
                ...
                "(consider `fusion-mlx rm` if unused models accumulated)",
                CheckStatus.WARN,
            ...
            s.add(
                ...
                "(consider `fusion-mlx rm` for unused models)",
                CheckStatus.WARN,
...
def _install_location() -> tuple[str, Path]:
    """Classify where ``fusion-mlx`` is installed: ``uv tool``, ``pipx``,
...
def section_python() -> Section:
    ...
    # user clearly if they somehow got fusion-mlx onto an older interpreter
    ...
    if sys.version_info >= (3, 10):  # noqa: UP036
        ...
        s.add(
            f"Python {py_ver} (fusion-mlx requires >= 3.10)",
            CheckStatus.FAIL,
            detail=f"executable={sys.executable}",
        )
...
def section_optional_packages() -> Section:
    ...
    # `pip install 'fusion-mlx[dflash]'` will actually work.
    dflash_min = (0, 5, 0)
    ...
    if vlm_ver and _version_at_least(vlm_ver, dflash_min):
        ...
        s.add(
            ...
            CheckStatus.WARN,
            detail="pip install 'fusion-mlx[dflash]'",
        )
...
_ARGCOMPLETE_HOOK_NEEDLE = "register-python-argcomplete fusion-mlx"
...
def section_shell_integration(
    *,
    ...
    cli_path = which_fn("fusion-mlx")
    if cli_path:
        s.add(
            f"fusion-mlx in $PATH ({cli_path})",
            CheckStatus.OK,
            detail=f"path={cli_path}",
        )
    else:
        s.add(
            "fusion-mlx NOT on $PATH",
            CheckStatus.FAIL,
            detail="shutil.which('fusion-mlx') returned None",
        )
    ...
    if present:
        ...
        s.add(
            ...
            '`eval "$(register-python-argcomplete fusion-mlx)"` to your shell rc',
            CheckStatus.WARN,
...
def section_optional_tools(
    *, which: Callable[[str], str | None] | None = None
    ...
    """Probe for development tools that improve the fusion-mlx experience but
...
# model stable across fusion-mlx versions.
_SECTION_BUILDERS = (
    section_system,
    section_python,
