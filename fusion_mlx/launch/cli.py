...
"""``fusion-mlx launch`` subcommand wiring.

This module exposes:

* :func:`register` — called from :mod:`fusion_mlx.cli` to wire the
...
* ``fusion-mlx launch list`` — print the supported clients + detection
  matrix. No state mutated.
* ``fusion-mlx launch <client>`` — patch the named client's config.
* ``fusion-mlx launch --all`` — patch every *detected* client.
...
``--start-server`` spawns ``fusion-mlx serve <model> --port <port>`` in
the background and writes the PID to ``~/.fusion-mlx/launch.pid`` so a
later ``kill $(cat ~/.fusion-mlx/launch.pid)`` shuts it down cleanly.
"""
...
PID_FILE = Path.home() / ".fusion-mlx" / "launch.pid"
...
def _resolve_default_model() -> str:
    ...
    * ``FUSION_MLX_DEFAULT_MODEL`` env var (lets the operator pin one)
...
def launch_command(args: argparse.Namespace) -> None:
    """Argparse entry point for ``fusion-mlx launch``.
    ...
    * otherwise → patch the named client.
    ...
    if not args.all and not args.client:
        print(
            ...
            "Try `fusion-mlx launch list` to see supported clients.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if args.all:
        ...
        if not targets:
            print(
                ...
                "Run `fusion-mlx launch list` to see what's checked.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
    ...
    # id. The top-level ``main()`` in ``fusion_mlx/cli.py`` rewrites
    ...
    # fusion-mlx (the server's ``/v1/models`` advertises the alias, and
    # request-side resolution accepts both), so we restore it here.
    # Same pattern as ``share_command`` in ``fusion_mlx/share/cli.py``.
    ...
    if args.dry_run:
        ...
        if args.start_server:
            print(f"[dry-run] would spawn: fusion-mlx serve {model} --port {args.port}")
        return
    ...
    if args.start_server:
        if not succeeded:
            ...
            print(f"  Started: fusion-mlx serve {model} --port {args.port} (pid {pid})")
    ...
    if succeeded:
        print(
            "\nNow ready: open "
            + " / ".join(succeeded)
            + " and it'll route through fusion-mlx."
        )
    if failures:
        sys.exit(1)
...
def register(subparsers) -> None:
    ...
    p = subparsers.add_parser(
        "launch",
        help="One-shot bootstrap: patch IDE/agent client config to use fusion-mlx",
        description=(
            "Detect an IDE client (Cline, Claude Code, Continue, Cursor) "
            "and write/patch its local config to route at the local "
            "fusion-mlx server. Use `fusion-mlx launch list` to see what's "
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Model alias the client will request from fusion-mlx "
            "(default: $FUSION_MLX_DEFAULT_MODEL or qwen3.5-4b-4bit)."
        ),
    )
    p.add_argument(
        "--server-url",
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Also spawn `fusion-mlx serve <model> --port <port>` in the "
            "background, writing the pid to ~/.fusion-mlx/launch.pid."
        ),
    )
    p.add_argument(
        "--dry-run",
