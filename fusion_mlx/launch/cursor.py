...
# Silicon-first; the fusion-mlx target platform) and Linux (a small but
# growing fraction of Cursor users since the official Linux build
# shipped). Windows isn't supported by fusion-mlx and so isn't probed.
...
def write_or_patch_config(
    ...
    """Patch Cursor's ``settings.json`` to point at the local fusion-mlx
