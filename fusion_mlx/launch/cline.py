...
``fusion-mlx launch cline`` has to do.
...
def _candidate_settings_roots() -> list[Path]:
    ...
    paths come first because that's the platform fusion-mlx targets
...
def write_or_patch_config(
    ...
    fusion-mlx OpenAI-compatible server.
    ...
    * ``openAiApiKey`` → ``<api_key>`` (default: ``"sk-noop"``,
      since fusion-mlx defaults to no-auth on loopback)
    * ``openAiModelId`` → ``<model>``
