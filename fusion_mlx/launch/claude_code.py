...
fusion-mlx already serves the Anthropic ``/v1/messages`` shape (see the
...
def write_or_patch_config(
    ...
    fusion-mlx Anthropic-compatible endpoint.
    ...
    * ``env.ANTHROPIC_MODEL`` → ``<model>`` (Claude Code reads this as
      the default model id; the fusion-mlx server accepts any
