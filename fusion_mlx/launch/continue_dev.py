...
``fusion-mlx`` and make it the default. On a re-run we de-dupe by name
...
# model named ``fusion-mlx`` on a re-run gets it updated in place rather
# than duplicated.
_MODEL_ENTRY_NAME = "fusion-mlx"
...
def detect() -> bool:
    ...
    still want ``fusion-mlx launch continue-dev`` to succeed by creating
...
def write_or_patch_config(
    ...
    """Insert (or update) a ``fusion-mlx`` model entry in Continue's
    ...
    On a re-run, an existing entry with ``title == "fusion-mlx"`` is
