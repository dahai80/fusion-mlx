...
class MiniMaxH3Backend(VideoBackend):
    ...
    async def load_vae_encoder(self) -> None:
        if getattr(self, "_stage_vae_encoder", None) is not None:
            return
        from fusion_mlx.video.minimax_h3.generate import _resolve_subdir
        from fusion_mlx.video.minimax_h3.vae import H3VAEConfig, MiniMaxH3VideoVAE

        video_vae_dir = _resolve_subdir(self._model_name, "video_vae")
        ...
        logger.info(
            "minimax_h3: vae_encoder load vae=%s",
