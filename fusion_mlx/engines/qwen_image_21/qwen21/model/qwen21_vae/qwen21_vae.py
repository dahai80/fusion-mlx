...
from .qwen21_decoder import Qwen21Decoder
from .qwen21_encoder import Qwen21Encoder
...
class Qwen21VAE(nn.Module):
    ...
    def __init__(self):
        ...
        # fusion-mlx transparent-PNG extension (PR#741 parity). When true the
        # 4th VAE output channel (alpha) is preserved so _numpy_to_pil yields an
        # RGBA PIL image and PNG save keeps transparency. Default false: the
        # alpha channel carries edit-mask signal, not image content, so txt2img
        # output drops it (upstream PR#736 base behavior).
        self.transparent_output = False
    ...
    def decode(self, latents: mx.array) -> mx.array:
        ...
        # The 2.1 VAE has 4 output channels (RGBA); the alpha channel carries
        # edit masks, not image content, so image output drops it unless
        # transparent_output is set (RGBA transparent PNG path).
        if self.transparent_output:
            return decoded
        return decoded[:, :3, :, :]
