...
class MLXEmbeddingModel:
    ...
    def load(self):
        try:
            ...
            raise ImportError(
                'mlx-embeddings required. Install with: pip install "fusion-mlx[embedding]"'
            ) from exc
    ...
    def embed(
        ...
        if embeddings.ndim == 3:
            mean_emb = [
                embeddings[i].mean(axis=0).tolist() for i in range(embeddings.shape[0])
            ]
        else:
        ...
        return EmbeddingOutput(
            embeddings=mean_emb, total_tokens=total_tokens, dimensions=dims
        )
