...
class TTSEngine(BaseNonStreamingEngine):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                'mlx-audio required for TTS. Install with: pip install "fusion-mlx[audio]"'
            ) from exc
    ...
    async def synthesize(
        ...
        **kwargs,
        ...
        activity_id = self._begin_activity(
            "synthesizing speech", metadata={"text_length": len(text)}
        )
        try:
            loop = asyncio.get_running_loop()
    ...
    async def stream_synthesize_pcm(
        ...
        **kwargs,
        ...
        activity_id = self._begin_activity(
            "streaming speech", metadata={"text_length": len(text)}
        )
        try:
            loop = asyncio.get_running_loop()
            while True:
                ...
                self._update_activity(
                    activity_id, chunk_count=chunk_count, output_bytes=total_bytes
                )
                yield sr, ch, sw, pcm
        finally:
            await self._finish_activity(activity_id)
