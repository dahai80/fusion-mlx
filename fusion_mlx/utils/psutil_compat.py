...
adapter in some bundled environments. fusion_mlx only needs a small subset
of ``psutil.virtual_memory()`` for memory guard and dashboard decisions, so
keep a local implementation for macOS and use psutil on other platforms.
"""
