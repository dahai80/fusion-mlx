...
from ..pool.model_discovery import _has_vision_subconfig
...
class OQManager:
    ...
    async def start_quantization(
        ...
        Args:
            ...
            oq_level: oQ level (2, 3, 4, 6, or 8). Ignored when recipe is set.
            recipe: MLX convert recipe name (mixed_3_4, quant2, etc.).
        ...
        if is_recipe and recipe not in self.MLX_RECIPES:
            raise ValueError(
                f"Unknown recipe {recipe!r}. Must be one of {sorted(self.MLX_RECIPES)}"
            )
        ...
        if not is_recipe and oq_level not in OQ_LEVELS:
            raise ValueError(
        ...
        if is_recipe:
            base = model_name.rstrip("-").rstrip("_")
            for suffix in ("mxfp8", "mxfp4", "bf16", "f16", "f32"):
                if base.endswith(f"-{suffix}") or base.endswith(f"_{suffix}"):
                    base = base[: -(len(suffix) + 1)]
                    break
            output_name = f"{base}-{recipe}"
            effective_level = 0.0
        else:
            output_name = resolve_output_name(
                model_name, oq_level, dtype, preserve_mtp=preserve_mtp
            )
            effective_level = oq_level
        ...
        for task in self._tasks.values():
            if task.model_path == model_path and task.status in _ACTIVE_STATUSES:
                if is_recipe and task.recipe == recipe:
                    raise ValueError(
                        f"Quantization for '{model_name}' with recipe {recipe!r} "
                        "is already in progress"
                    )
                if not is_recipe and task.oq_level == oq_level and task.dtype == dtype:
                    raise ValueError(
        ...
        if is_recipe:
            logger.info(
                f"Recipe quantization queued: {model_name} -> {recipe} "
                f"(task_id={task_id})"
            )
        else:
            logger.info(
    ...
    @staticmethod
    def _run_mlx_convert(
        model_path: str,
        output_path: str,
        recipe: str,
        dtype: str = "bfloat16",
    ) -> None:
        """Run mlx_lm.convert with a named recipe."""
        import subprocess
        import sys
    ...
    async def _run_quantization(self, task_id: str) -> None:
        ...
        try:
            async with self._quant_sem:
                ...
                # Recipe-based quantization path
                if task.recipe:
                    task.phase = f"Converting with {task.recipe}..."
                    task.progress = 5.0
                    ...
                    # oQ sensitivity-based quantization path
