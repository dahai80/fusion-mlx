...
"""ModelScope model downloader for Fusion-MLX admin panel.
...
def _get_ms_endpoint() -> str:
    ...
    try:
        ...
        if sm and sm.settings:
            ms_cfg = sm.settings.global_settings.get("modelscope", {})
            if isinstance(ms_cfg, dict):
                endpoint = ms_cfg.get("endpoint") or ""
                if endpoint:
                    return endpoint.rstrip("/")
...
class MSDownloader:
    ...
    async def get_model_info(model_id: str) -> dict:
        ...
        try:
            ...
            readme_url = (
                f"{endpoint}/api/v1/models/{model_id}/repo"
                f"?FilePath=README.md&Revision=master"
            )
            resp = await asyncio.wait_for(
                asyncio.to_thread(
                    requests.get,
        ...
        except Exception:  # noqa: BLE001 — README is optional
    ...
    async def start_download(self, model_id: str, ms_token: str = "") -> DownloadTask:
        ...
        if not MS_SDK_AVAILABLE:
            raise RuntimeError(
                ...
                'Install with: pip install "fusion-mlx[modelscope]"'
            )
        ...
        if "/" not in model_id or len(model_id.split("/")) != 2:
            raise ValueError(
                ...
                "(e.g., 'qwen/Qwen2.5-7B-Instruct-MLX')"
            )
    ...
    async def _run_download(self, task_id: str, ms_token: str) -> None:
        ...
        try:
            async with self._download_sem:
                ...
                # Note: Thread cannot be interrupted, cancellation is checked
                # after completion.
                ...
                if task_id in self._cancelled:
                    logger.info(
                        f"MS Download was cancelled during execution: "
                        f"{task.repo_id}. Cleaning up downloaded files..."
                    )
                    ...
                    # Drop empty org folder left behind by the cancelled
                    # download.
            ...
            if task.status not in (
                ...
                task.status = DownloadStatus.CANCELLED
        except Exception as e:  # noqa: BLE001 — surface to task.error
