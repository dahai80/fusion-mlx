...
"""Auto-detect optimal configuration for a model family — core detection.

ModelConfig dataclass + detect_model_config + family/path detection +
deepseek template helpers + enrich_model_config. Split from the original
model_auto_config.py monolith; profile formatting lives in profile.py.
"""
...
_MODEL_PATTERNS: list[tuple[re.Pattern, ModelConfig]] = [
    ...
    # TODO: fusion_mlx/tool_parsers/ui_tars_tool_parser does not exist yet in fusion_mlx
    # shape (see fusion_mlx.tool_parsers.ui_tars_tool_parser). MUST come
