...
class SDXLCLIPTextModel(nn.Module):
    # Single CLIP text model (CLIP-L or OpenCLIP-G). Returns last hidden
    # state (full sequence) + pooled (EOS-row). For CLIP-G an extra
    # text_projection Linear maps hidden->projection_dim before pooling
    # (matches HF CLIPTextModelWithProjection).
    ...
    def __init__(
        ...
        super().__init__()
        self.dims = dims
        self.has_projection = projection_dim is not None
        ...
        if self.has_projection:
            self.text_projection = nn.Linear(dims, projection_dim, bias=False)
