...
def _keep_home_env(
    ...
    A file in ``~/.fusion-mlx/agents/`` replaces a built-in profile *wholesale*,
...
def load_profiles(profiles_dir: Path | str | None = None):
    ...
    Args:
        profiles_dir: Directory containing .yaml profile files.
                      Defaults to fusion_mlx/agents/profiles/
    """
    global _LOADED
    _PROFILES.clear()
    ...
    user_profiles = Path(os.path.expanduser("~/.fusion-mlx/agents"))
