...
def _api_call(
    ...
    """Direct API call to fusion-mlx server."""
    payload = {
        "model": model_id,
        "messages": messages,
    ...
    try:
        ...
        with httpx.stream(
            ...
            for line in resp.iter_lines():
                ...
                # `reasoning` (fusion_mlx/routes/chat.py:1434) — the prior
...
def _test_tag_suppression(
    ...
    t0 = time.time()
    try:
        from fusion_mlx.api.utils import StreamingToolCallFilter
...
def _agent_query(
    ...
    try:
        ...
        # mismatch, not a fusion-mlx regression. Propagate as SKIP via the
...
def _err_to_status(err: str | None) -> TestStatus:
    ...
    - ``"SKIP:"`` prefix → SKIP. ``_agent_query`` propagates this for
      harness-side init refusals where fusion-mlx has nothing to fix —
...
def _test_e2e_terminal(
    ...
    t0 = time.time()
    marker = f"fusionmlx_{agent_name}_test"
    out, err = _agent_query(
...
class AgentTestRunner:
    ...
    def _server_available(self) -> bool:
        """Check if the fusion-mlx server is running."""
        try:
            r = httpx.get(
    ...
    def run(self) -> TestReport:
        ...
        if not self._server_available():
            report.results.append(
                TestResult(
                    "server_check",
                    TestStatus.ERROR,
                    message="fusion-mlx server not running",
                )
            )
            return report
        ...
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — config refresh must never abort the sweep
            logger.warning(
    ...
    def _run_specific_tests(self, test_module_name: str) -> list[TestResult]:
        ...
        # (fusion_mlx/agents/profiles/*.yaml), so this is defense-in-depth
        ...
        try:
            ...
            os.environ["FUSION_MLX_BASE_URL"] = self.base_url
            ...
            if exec_error is not None:
                ...
                # the harness deps weren't installed in the fusion-mlx
                    ...
                    try:
                        ...
                        if testing and testing.install_cmd:
                            hint = (
                                ...
                                f"`{testing.install_cmd}` in the fusion-mlx venv]"
                            )
