    ...
    *,
    ...
    if cfg.path:
        ...
        if profile.name == "codex" and isinstance(rendered, str):
            ...
            catalog_path = config_path.parent / "fusion-mlx-model-catalog.json"
...
def _merge_toml(existing_text: str, rendered: str) -> str:
    ...
    profile, so one ``fusion-mlx agents codex --setup`` deleted the user's
...
def get_setup_instructions(
    ...
    *,
    ...
    lines = [
        f"# {profile.display_name} + fusion-mlx Setup",
        "",
        "## 1. Start fusion-mlx",
        "",
    ]
    ...
    if profile.recommended_models:
        lines.append("```bash")
        cmd = f"fusion-mlx serve {serve_model}"
        ...
        lines.append("```bash")
        lines.append("fusion-mlx serve <MODEL>")
        lines.append("```")
...
def apply_streaming_config(profile: AgentProfile, agent_version: str | None = None):
    ...
    """
    from fusion_mlx.api.utils import register_tool_call_tag
...
def get_adapter(*args, **kwargs):
    ...
    The adapter surface is the module-level functions (setup_agent_config,
    get_setup_instructions, apply_streaming_config). This stub exists for
    callers that imported ``get_adapter`` from the prior stub layout.
    """
    return None
