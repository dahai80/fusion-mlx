...
class PriorityScheduler:
    ...
    def submit(
        ...
        **kwargs,
        ...
        logger.debug(
            ...
            f"queue_depth={self._queues[priority].__len__()}"
        )
        return request_id
    ...
    def step(self) -> Any:
        ...
        1. Check for preemption (REALTIME waiting while BATCH running)
        2. Drain priority queues to base scheduler (REALTIME > BATCH > BACKGROUND)
        3. Run base scheduler step
        4. Update starvation counters
        """
        self._steps += 1

        # Preemption check: if REALTIME queue has items, pause BATCH prefill
        if (
            self.config.preempt_batch_for_realtime
            and self._queues[PriorityLevel.REALTIME]
        ):
            self._maybe_preempt()

        # Chunked prefill: check for preemption between chunks
        self._check_chunked_prefill()

        # Drain queues to base scheduler (priority order)
        scheduled_this_step = 0
        for pl in PriorityLevel:
            n = self._drain_to_base(
                max_requests=self._get_max_for_level(pl),
                priority_filter=pl,
                )
        ...
        # Starvation rescue: force-schedule BACKGROUND if starved
        self._check_starvation()

        # Run base scheduler step
    ...
    def has_requests(self) -> bool:
        return self.base.has_requests() or any(
    ...
    def get_stats(self) -> dict[str, Any]:
        return {
                "queue_depths": {
                pl.name: len(q) for pl, q in self._queues.items()
                },
                ...
                "streams": {
                pl.name: str(s) for pl, s in self._streams.items()
                },
        }
    ...
    def _drain_to_base(self, max_requests: int = 1, priority_filter: PriorityLevel | None = None) -> int:
        ...
        for pl in levels_to_check:
            ...
            while q and scheduled < max_requests:
                ...
                # Pop and submit to base
                pri_req = q.popleft()

                # Set stream on base scheduler if supported
    ...
    def _maybe_preempt(self) -> None:
        ...
        Strategy:
        - If BATCH requests are in prefill (not yet generating tokens), pause them
        - If BACKGROUND requests are running, abort them
        - Never preempt REALTIME (by definition highest priority)
        """
        # Check if there are BATCH/BACKGROUND requests that can be preempted
        if not hasattr(self.base, "running"):
            return

        # Count running requests per priority
        running_counts = {pl: 0 for pl in PriorityLevel}
        for rid, pl in self._request_priorities.items():
            running_counts[pl] += 1

        # Preempt BACKGROUND for any higher priority waiting
        if (
            self.config.preempt_background_for_any
            and running_counts[PriorityLevel.BACKGROUND] > 0
            and (self._queues[PriorityLevel.REALTIME] or self._queues[PriorityLevel.BATCH])
        ):
            self._preempt_at_level(PriorityLevel.BACKGROUND)

        # Preempt BATCH only for REALTIME
        if (
            self.config.preempt_batch_for_realtime
            and running_counts[PriorityLevel.BATCH] > 0
            and self._queues[PriorityLevel.REALTIME]
        ):
            # Only preempt if we have significant realtime backlog
    ...
    def _check_chunked_prefill(self) -> None:
        ...
        if not self._queues[PriorityLevel.REALTIME]:
            return  # No realtime pressure, no need to chunk

        if not hasattr(self.base, "running"):
            return

        # Check if any running BATCH request has a long prefill
        for rid in list(self._request_priorities.keys()):
            if self._request_priorities[rid] != PriorityLevel.BATCH:
                continue
            ...
            try:
                ...
                if req is None:
                    continue
                # If the request prompt is longer than chunk size, it's a candidate
                ...
                if token_est > self.config.prefill_chunk_size:
                    ...
                    # Signal base scheduler to process in chunks
    ...
    def _check_starvation(self) -> None:
        ...
        for pl in [PriorityLevel.BACKGROUND, PriorityLevel.BATCH]:
            if (
                self._steps_since_served[pl] >= self.config.starvation_threshold_steps
                and self._queues[pl]
            ):
                # Force-drain at least 1 from this level
                ...
                if n > 0:
                    ...
                    logger.info(
                        ...
                        f"scheduled 1 {pl.name} request after "
                        f"{self._steps_since_served[pl]} steps"
                        )
