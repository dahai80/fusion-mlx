...
DIFFUSERS_PIPELINE_TASKS = {
    ...
    "FluxControlNetPipeline": "text-to-image",
    "FluxDepthPipeline": "text-to-image",
    "FluxFillPipeline": "text-to-image",
    "FluxKontextPipeline": "text-to-image",
    "FluxReduxPipeline": "text-to-image",
...
def _is_comfy_flux2_layout(path: Path) -> bool:
    ...
    has_vae = any(
        p.suffix == ".safetensors" for p in _iter_safetensors_entries(vae)
    )
...
def _iter_safetensors_entries(dir_path: Path):
    ...
    try:
        for p in dir_path.iterdir():
            if p.name.endswith(".safetensors") and (
...
def _is_hf_cache_mlx_compatible(model_dir: Path, source_repo_id: str) -> bool:
    ...
    # _is_comfy_*_layout, so accept before that gate.
    if _is_comfy_flux2_layout(model_dir) or _is_comfy_ltx2_5_layout(model_dir):
        logger.info(
            f"Accepting HF cache Comfy image/video layout: {source_repo_id}"
        )
        return True
    ...
    if not list(model_dir.glob("model*.safetensors")):
        ...
        if list(model_dir.glob("*.npz")):
            logger.info(
                f"Accepting HF cache model with MLX .npz weights: {source_repo_id}"
            )
            return True
...
class ModelDiscovery:
    """Wrapper for model discovery from a directory."""
