...
FusionMLX already injects the DeepSeek-V4 base model itself via
``fusion_mlx/patches/deepseek_v4/`` (which lands the model class into
...
def apply() -> bool:
    ...
    if not hasattr(dsv4.Model, "_fusion_mlx_mtp_patched"):
        dsv4.Model._fusion_mlx_mtp_patched = "patch"
...
def _patch_model_args(dsv4: Any) -> None:
    ...
    if "_fusion_mlx_mtp_args_patched" in args_cls.__dict__:
        return
    ...
    args_cls._fusion_mlx_mtp_args_patched = True
...
def _patch_model(dsv4: Any) -> None:
    ...
    # FusionMLX's DeepSeek-V4 fork uses a ``CacheList`` of (RotatingKVCache,
    # PoolingCache, [PoolingCache]) instead of upstream's
    # ``DeepseekV4Cache`` wrapper. The make_mtp_cache + mtp_forward bodies
    # below use the FusionMLX-side cache layout to stay compatible with the
    ...
    def sanitize(self, weights: dict[str, Any]) -> dict[str, Any]:
        ...
        # FP4 dequant pre-pass (FusionMLX-specific). Identical to the
