...
This module is the mlx-vlm-side companion to ``fusion_mlx/patches/mlx_lm_mtp``.
It adds:
...
* a ``sanitize`` extension that keeps ``mtp.*`` weights and converts the
  ...
  uses at runtime;
* matching pass-through methods on ``fusion_mlx.models.vlm.VLMModelAdapter`` so
...
(``fusion_mlx/utils/model_loading.py``) calls ``apply_mlx_vlm_mtp_runtime_patch()``
...
def _patch_text_config(q35moe_config: Any) -> None:
    ...
    if getattr(cls, "_fusion_mlx_mtp_from_dict_patched", False):
        return
    ...
    cls._fusion_mlx_mtp_from_dict_patched = True
...
def _patch_vlm_language_model(q35moe_lang: Any) -> None:
    ...
    if "_fusion_mlx_mtp_runtime_patched" in cls.__dict__:
        return
    ...
    def __init__(self, args, config=None):
        ...
        # ``_fusion_mlx_mtp_decode_enabled`` marker.
        #
        ...
        self._fusion_mlx_mtp_decode_enabled = bool(
    ...
    cls._fusion_mlx_mtp_runtime_patched = True
...
def _patch_vlm_model_adapter() -> None:
    """Extend ``fusion_mlx.models.vlm.VLMModelAdapter`` with MTP plumbing.
    ...
    if getattr(VLMModelAdapter, "_fusion_mlx_mtp_adapter_patched", False):
        return
    ...
    VLMModelAdapter._fusion_mlx_mtp_adapter_patched = True
...
def _patch_vlm_outer_model_sanitize(q35moe_outer: Any) -> None:
    ...
    if "_fusion_mlx_mtp_runtime_sanitize_patched" in cls.__dict__:
        return
    ...
    def sanitize(self, weights):
        ...
        for key, value in weights.items():
            ...
            if "conv1d.weight" in key and value.shape[-1] != 1:
                ...
                # streaming-discovery monkey-patch (in ``fusion_mlx.oq``) when
    ...
    cls._fusion_mlx_mtp_runtime_sanitize_patched = True
