...
Vendors the FusionMLX GLM-5.2 optimized mlx-lm model code without modifying the
...
the optimized helper modules remain private to ``fusion_mlx.patches.glm_moe_dsa`` and
...
NATIVE_KERNELS_PACKAGE = "fusion_mlx.custom_kernels.glm_moe_dsa"
...
def _register_module() -> None:
    ...
    logger.info("Registered %s from FusionMLX optimized vendored module", qualname)
...
def apply_glm_moe_dsa_patch() -> bool:
    ...
    Returns True when FusionMLX registered its optimized vendored module, False when
