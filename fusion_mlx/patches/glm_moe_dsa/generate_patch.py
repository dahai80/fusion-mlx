...
def apply_glm_moe_dsa_generate_patch() -> bool:
    ...
    def patched_ppb_init(self, *args, **kwargs):
        ...
        self._fusion_mlx_glm_dsa_adaptive_prefill = _glm_dsa_adaptive_prefill_config(
            model, prefill_step_size
        )

    def patched_ppb_copy(self):
        new_batch = original_ppb_copy(self)
        new_batch._fusion_mlx_glm_dsa_adaptive_prefill = getattr(
            self, "_fusion_mlx_glm_dsa_adaptive_prefill", None
        )
        return new_batch

    def patched_ppb_split(self, indices):
        new_batch = original_ppb_split(self, indices)
        if not hasattr(new_batch, "_fusion_mlx_glm_dsa_adaptive_prefill"):
            new_batch._fusion_mlx_glm_dsa_adaptive_prefill = getattr(
                self, "_fusion_mlx_glm_dsa_adaptive_prefill", None
            )
        return new_batch
    ...
    def patched_ppb_prompt(self, tokens):
        adaptive_prefill = getattr(self, "_fusion_mlx_glm_dsa_adaptive_prefill", None)
    ...
    def patched_bg_init(self, *args, **kwargs):
        ...
        self._prompt_batch._fusion_mlx_glm_dsa_adaptive_prefill = (
            self._fusion_mlx_glm_dsa_adaptive_prefill
        )
    ...
    patched_generate_step._fusion_mlx_glm_dsa_adaptive_patched = True
    gen.generate_step = patched_generate_step

    PromptProcessingBatch._fusion_mlx_glm_dsa_adaptive_patched = True
    BatchGenerator._fusion_mlx_glm_dsa_adaptive_patched = True
    _APPLIED = True
