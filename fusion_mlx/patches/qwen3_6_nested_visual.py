...
The mlx_vlm_mtp runtime sanitize (fusion_mlx/patches/mlx_vlm_mtp) re-implements
...
``fusion_mlx.utils.model_loading.maybe_apply_pre_load_patches`` (after
``apply_mlx_vlm_mtp_runtime_patch`` so it covers whichever sanitize the
class currently has) and from ``fusion_mlx.oq._build_model_sanitizer``.
...
def _make_patched_sanitize(original_sanitize):
    ...
    patched_sanitize._fusion_mlx_nested_visual_wrapped = True
...
def apply_qwen3_6_nested_visual_patch() -> bool:
    ...
    ``_fusion_mlx_nested_visual_wrapped`` marker. Uses a function-attribute marker
    ...
    if getattr(original_sanitize, "_fusion_mlx_nested_visual_wrapped", False):
        return False
