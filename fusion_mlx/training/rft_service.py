...
"""RFT (rejection-sampling fine-tuning) training service.

Importers/callers:
  - fusion_mlx.admin.fine_tune_route imports RFTService + RFTJob for routes
  - fusion_mlx.server instantiates + wires via set_rft_context

Affected API: new /admin/api/fine-tune/rft/* endpoints (POST create, GET
list/stream, POST cancel, DELETE).

Data schemas:
  RFTJob   — job record (job_id, model_id, prompts, config, status, events[], cond)
  RFTService — singleton service (queue, CRUD, _execute_rft, SSE progress)

Mirrors GRPOService. RFT samples completions (like GRPO), so it inherits the
same #751 generation_stream worker-thread rebind fix.
"""
...
from fusion_mlx.training.rft import RFTConfig, RFTTrainer
...
class RFTJob:
    ...
    config: RFTConfig = field(default_factory=RFTConfig)
    adapter_name: str = ""
    adapter_path: str = ""
...
class RFTService:
    # Singleton managing RFT job queue + execution. One concurrent job
    # (Apple Silicon memory constraint — training evicts inference model).

    def __init__(self):
        self._jobs: dict[str, RFTJob] = {}
    ...
    async def shutdown(self):
        ...
        self._running = False
        if cancelled:
            logger.info("RFTService shutdown: cancelled %d pending task(s)", cancelled)
    ...
    def create_job(
        ...
        model_id: str,
        prompts: list,
        config: RFTConfig | None = None,
        adapter_name: str = "",
    ) -> RFTJob:
        cfg = config or RFTConfig()
        adapter_name = adapter_name or f"rft-{uuid.uuid4().hex[:6]}"
        ...
        self._persist_jobs()
        logger.info(
            "RFT job created: %s model=%s adapter=%s queued=%d",
            job.job_id,
            model_id,
            adapter_name,
            len(self._queue),
    ...
    def get_job(self, job_id: str) -> RFTJob | None:
        return self._jobs.get(job_id)

    def list_jobs(self) -> list[RFTJob]:
    ...
    def start_processing(self):
        ...
        self._process_queue()
        logger.info("RFT service processing queue (jobs=%d)", len(self._queue))
    ...
    def _process_queue(self):
        ...
        if self._loop is None:
            logger.error("No event loop for RFT execution")
    ...
    async def _run_job(self, job: RFTJob):
        try:
            await asyncio.to_thread(self._execute_rft, job)
        except Exception as exc:
            logger.exception("RFT job %s failed: %s", job.job_id, exc)
            job.status = JobStatus.FAILED
            job.error = str(exc)
            job.terminal = True
            job.finished_at = time.time()
    ...
    def _execute_rft(self, job: RFTJob):
        # Run RFT training in a background thread (blocking). RFT samples
        # completions (like GRPO), so it needs the same #751 generation_stream
        # worker-thread rebind — mlx_lm.generate's module-level stream is
        # captured at import on the main thread and is not bound here.
        import importlib
        ...
        logger.info(
            "RFT execute(worker): rebinding generation_stream job=%s", job.job_id
        )
    ...
    def _run_rft(self, job: RFTJob):
        ...
        logger.info("RFT execute: model=%s job=%s", model_path, job.job_id)
        ...
        if n_prompts == 0:
            raise ValueError("RFT job has no prompts")
        ...
        logger.info("RFT job %s completed: adapter=%s", job.job_id, job.adapter_path)

    def _push_event(self, job: RFTJob, event: dict):
    ...
    def _notify_job(self, job: RFTJob):
        async def _do():
            async with job.cond:
                job.cond.notify_all()
    ...
    def _jobs_file(self) -> Path:
        return ADAPTER_BASE_DIR / "rft_jobs.json"
    ...
    def _persist_jobs(self):
        ...
        try:
            ...
            logger.warning("Failed to persist RFT jobs: %s", exc)
    ...
    def _load_jobs(self):
        ...
        try:
            ...
            logger.warning("Failed to load RFT jobs: %s", exc)
            return
        for item in data:
            try:
                cfg = RFTConfig(**item.get("config", {}))
                ...
                logger.warning("Skipping malformed RFT job: %s", exc)
