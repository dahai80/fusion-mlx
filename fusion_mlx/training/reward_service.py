...
"""Reward model training service (#424).

Importers/callers:
  - fusion_mlx.admin.fine_tune_route imports RewardService + RewardJob for routes
  - fusion_mlx.server instantiates + wires via set_reward_context

Affected API: new /admin/api/fine-tune/reward/jobs endpoints
  (POST create, GET list/{id}, POST cancel, DELETE)

Data schemas:
  RewardJob     — job record (job_id, model_id, preference_pairs, config, events[], cond)
  RewardService — singleton service (queue, CRUD, _execute_reward, SSE progress)
...
class RewardService:
    # Singleton managing reward-model job queue + execution. One concurrent job
    # (Apple Silicon memory constraint — training evicts inference model).

    def __init__(self):
        self._jobs: dict[str, RewardJob] = {}
    ...
    def create_job(
        ...
        adapter_name = adapter_name or f"reward-{uuid.uuid4().hex[:6]}"
        ...
        self._persist_jobs()
        logger.info(
            "REWARD job created: %s model=%s adapter=%s queued=%d",
            job.job_id,
            model_id,
            adapter_name,
            len(self._queue),
    ...
    def get_job(self, job_id: str) -> RewardJob | None:
        return self._jobs.get(job_id)

    def list_jobs(self) -> list[RewardJob]:
    ...
    def start_processing(self):
        ...
        self._process_queue()
        logger.info("Reward service processing queue (jobs=%d)", len(self._queue))
    ...
    def _process_queue(self):
        ...
        if self._loop is None:
            logger.error("No event loop for reward execution")
    ...
    async def _run_job(self, job: RewardJob):
        try:
            await asyncio.to_thread(self._execute_reward, job)
        except Exception as exc:
            logger.exception("Reward job %s failed: %s", job.job_id, exc)
            job.status = JobStatus.FAILED
            job.error = str(exc)
            job.terminal = True
            job.finished_at = time.time()
    ...
    def _execute_reward(self, job: RewardJob):
        # Run reward-model training in a background thread (blocking). Load
        # model, apply LoRA, run preference-pair loop, save adapter, cleanup.
        ...
        cfg = job.config
        logger.info("REWARD execute: model=%s job=%s", model_path, job.job_id)
        ...
        trainer = RewardTrainer(model, tokenizer, model_path, cfg)

        pairs = job.preference_pairs
        batch_size = max(cfg.batch_size, 1)
        n_pairs = len(pairs)
        if n_pairs == 0:
            raise ValueError("reward job has no preference_pairs")
        ...
        logger.info(
            "REWARD job %s completed: adapter=%s",
            job.job_id,
            job.adapter_path,
        )

    def _push_event(self, job: RewardJob, event: dict):
    ...
    def _notify_job(self, job: RewardJob):
        async def _do():
            async with job.cond:
                job.cond.notify_all()
    ...
    def _jobs_file(self) -> Path:
        return ADAPTER_BASE_DIR / "reward_jobs.json"
    ...
    def _persist_jobs(self):
        ...
        try:
            ...
            logger.warning("Failed to persist reward jobs: %s", exc)
    ...
    def _load_jobs(self):
        ...
        try:
            ...
            logger.warning("Failed to load reward jobs: %s", exc)
            return
        for item in data:
            try:
                cfg = RewardConfig(**item.get("config", {}))
                ...
                logger.warning("Skipping malformed reward job: %s", exc)
