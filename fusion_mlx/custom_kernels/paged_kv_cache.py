...
class FusionPagedKVCache:
    ...
    def __init__(
        ...
        # D1 (audit): standalone cache is per-request/single-threaded in
        # practice, but the audit flags the whole file. RLock guards
        # free_list/block_table mutation so a concurrent trim+fetch can't
        # pop the same free block or iterate a mutating block_table.
    ...
    def _ensure_pool(
        ...
        self._dtype = dtype
        # D9 (audit): re-allocating keys_pool/values_pool drops the old
        # tensors to GC. Under rapid geometry changes the old allocations
        # linger until GC runs -> transient GPU memory spike. Force a
        # collection so the old pool is released before the new one fills.
    ...
    def _free_block(self, pb: int) -> None:
        # D6 (audit): single-block free routed through free_list under the
        # lock so trim/rollback stay consistent with _alloc_block.
    ...
    def update_and_fetch(self, keys, values):
        ...
        # mid-operation alloc failure rolls them back — otherwise
        # block_table holds allocated blocks with a stale offset.
        ...
        with self._lock:
            try:
                ...
                # D3 (audit): rollback tail blocks allocated this call.
                while self.block_table and self.block_table[-1] in allocated_this_call:
                    self._free_block(self.block_table.pop())
                logger.warning(
                    "paged_kv update_and_fetch rolled back %d blocks "
                    "(offset unchanged at %d)",
    ...
    def state(self, v):
        ...
        with self._lock:
            # D8 (audit): free old block_table entries explicitly before
            # rebuilding free_list, so block ids are tracked consistently.
