...
namespace fusion_mlx::glm_kernels {
...
std::string current_binary_dir() {
  static std::string binary_dir = []() {
    ...
    if (!dladdr(reinterpret_cast<void*>(&current_binary_dir), &info)) {
      throw std::runtime_error("Unable to get fusion_mlx_glm_kernels binary dir.");
    }
...
class GlmDsaQ8VupFlatPrimitive : public Primitive {
 public:
  ...
  void eval_gpu(
    ...
    auto lib = d.get_library("fusion_mlx_glm_kernels", current_binary_dir());
...
array glm_dsa_q8_vup_flat(
  ...
  if (x.ndim() != 4 || weight.ndim() != 3 || scales.ndim() != 3 ||
      biases.ndim() != 3) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_q8_vup_flat] expected x rank 4 and "
  ...
  if (H != weight.shape(0) || H != scales.shape(0) ||
      ...
      biases.shape(2) != K / group_size) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_q8_vup_flat] incompatible shapes: "
  ...
  if (x.dtype() != float16 && x.dtype() != bfloat16) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_q8_vup_flat] expected float16 or "
  ...
  if (weight.dtype() != uint32 || scales.dtype() != x.dtype() ||
      biases.dtype() != x.dtype()) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_q8_vup_flat] expected uint32 weight and "
  ...
  if (GlmDsaQ8VupFlatPrimitive::unsupported(x, weight, scales, biases, stream)) {
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.glm_dsa_q8_vup_flat] unsupported M3 GLM shape.");
  }
...
array glm_moe_weighted_sum(
  ...
  if (x_sorted.ndim() != 3 || x_sorted.shape(-2) != 1) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] expected x_sorted shape "
        << "[N, 1, D], got " << x_sorted.shape() << ".";
    throw std::invalid_argument(msg.str());
  }
  if (scores.ndim() < 2) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] expected scores rank >= 2, "
        << "got " << scores.shape() << ".";
    throw std::invalid_argument(msg.str());
  }
  if (inv_order.ndim() != 1 || inv_order.dtype() != uint32) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] expected uint32 inv_order "
  ...
  if (x_sorted.shape(0) != routed_rows || inv_order.size() != routed_rows) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] incompatible shapes: "
  ...
  if (topk <= 0 || D <= 0) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] invalid topk or hidden "
        << "dim: topk=" << topk << ", D=" << D << ".";
    throw std::invalid_argument(msg.str());
  }
  if (!issubdtype(x_sorted.dtype(), floating)) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] expected floating "
  ...
  if (GlmMoeWeightedSumPrimitive::unsupported(
    ...
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.glm_moe_weighted_sum] unsupported M3 GLM shape.");
  }
  return array(
      std::move(out_shape),
      x_sorted.dtype(),
