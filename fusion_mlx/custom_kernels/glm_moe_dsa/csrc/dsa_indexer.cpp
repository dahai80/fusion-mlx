...
array dsa_topk_indices_impl(
  ...
  if (scores.ndim() != 4 || scores.shape(1) != 1) {
    ...
    msg << "[fusion_mlx_glm_kernels.dsa_topk_indices] expected scores with shape "
        << "[B, 1, L, K], got " << scores.shape() << ".";
    throw std::invalid_argument(msg.str());
  }
  if (topk <= 0 || topk > scores.shape(-1)) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_topk_indices] invalid topk " << topk
  ...
  if (DSATopKIndicesPrimitive::unsupported(scores_contiguous, topk, stream)) {
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.dsa_topk_indices] unsupported M3 GLM shape.");
  }
...
array dsa_indexer_scores(
  ...
  if (queries.ndim() != 4 || keys.ndim() != 4 ||
      (weights.ndim() != 3 && weights.ndim() != 4)) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_indexer_scores] expected q/k rank 4 and "
  ...
  if (keys.shape(1) != 1) {
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.dsa_indexer_scores] keys must have a singleton "
        "indexer head axis.");
  }
  const bool weights_lh = weights.ndim() == 3;
  ...
  if (queries.shape(0) != keys.shape(0) ||
    ...
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_indexer_scores] incompatible q, k, "
  ...
  if (final_type != float16 && final_type != bfloat16) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_indexer_scores] expected float16 or "
        << "bfloat16 inputs, got " << final_type << ".";
    throw std::invalid_argument(msg.str());
  }
  if (unused_causal_prefix_topk < 0) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_indexer_scores] "
  ...
  if (causal_q_offset < -1) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.dsa_indexer_scores] causal_q_offset must be "
  ...
  if (DSAIndexerScoresPrimitive::unsupported(q, k, w, stream)) {
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.dsa_indexer_scores] unsupported M3 GLM shape.");
  }
