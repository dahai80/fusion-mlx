...
array glm_dsa_sparse_mla_attention(
  ...
  for (const auto& tensor : {q_latent, q_pe, kv_latent, k_pe}) {
    if (tensor.ndim() != 4) {
      ...
      msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] input with shape "
  ...
  if (topk_indices.ndim() != 4) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] topk_indices with "
        << "shape " << topk_indices.shape() << " expected to be rank 4.";
    throw std::invalid_argument(msg.str());
  }
  if (causal_prefix_rows < 0 || causal_prefix_rows > q_latent.shape(2)) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] "
  ...
  if (q_latent.shape(0) != q_pe.shape(0) ||
    ...
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] incompatible "
  ...
  if (topk_indices.dtype() != uint32) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] topk_indices must "
  ...
  if (topk_length.has_value()) {
    ...
    if (!shape_ok) {
      std::ostringstream msg;
      msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] topk_length "
    ...
    if (lengths.dtype() != uint32) {
      std::ostringstream msg;
      msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] topk_length must "
  ...
  if (final_type != float16 && final_type != bfloat16 && final_type != float32) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] expected floating "
  ...
  if (GlmDsaSparseMlaAttentionPrimitive::unsupported(
    ...
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.glm_dsa_sparse_mla_attention] unsupported M3 GLM shape.");
  }
