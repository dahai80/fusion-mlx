...
class GlmDsaExactBlockAttentionPrimitive : public Primitive {
 public:
  ...
  void eval_gpu(
    ...
    concatenate(
        base_name,
        "fusion_mlx_glm_exact_attention_",
        type_to_name(q),
        "_bq",
        bq,
        "_bk",
...
array glm_dsa_exact_block_attention(
  ...
  for (const auto& tensor : {q, k, v}) {
    if (tensor.ndim() != 4) {
      ...
      msg << "[fusion_mlx_glm_kernels.glm_dsa_exact_block_attention] input with "
  ...
  if (final_type != float16 && final_type != bfloat16) {
    std::ostringstream msg;
    msg << "[fusion_mlx_glm_kernels.glm_dsa_exact_block_attention] expected fp16 "
  ...
  if (GlmDsaExactBlockAttentionPrimitive::unsupported(
    ...
    throw std::invalid_argument(
        "[fusion_mlx_glm_kernels.glm_dsa_exact_block_attention] unsupported GLM exact block SDPA shape.");
  }
