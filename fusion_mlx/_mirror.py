...
``fusion-mlx pull <alias>`` (and the implicit prefetch invoked by
``fusion-mlx serve <alias>`` / ``fusion-mlx chat <alias>`` when the model
...
# maintainer. Any plausible browser-ish UA works. Keep ``fusion-mlx`` in
# the string so the maintainer can spot our traffic in R2 logs.
_USER_AGENT = "Mozilla/5.0 (fusion-mlx mirror client)"
...
def _sidecar_key_for(relpath: str) -> str:
    ...
    lets a repository file legitimately named ``.foo.fusion-mlx-mirror
    .part`` (yes, file names can start with dots) collide with the
    temp file for ``foo``, and similarly for ``.lock``. Move sidecars
    into ``repo_root/.fusion-mlx-mirror/`` with a flattened key derived
...
def _download_one_from_r2(
    ...
    *,
    ...
    (``repo_root/.fusion-mlx-mirror/``) where the ``.part`` and ``.lock``
...
def _acquire_part_lock(lock_path: Path):
    ...
    Codex round-11 BLOCKING #2: prevents two concurrent ``fusion-mlx``
    processes from racing on the same ``<file>.fusion-mlx-mirror.part``.
    ...
    try:
        ...
        # fusion-mlx use case is MLX-only (macOS), so this branch is
