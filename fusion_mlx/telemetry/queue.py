...
queue must serve both ``fusion-mlx serve`` (async FastAPI) and
``fusion-mlx chat`` (sync REPL) without forcing every caller into an
...
class TelemetryQueue:
    ...
    def __init__(
        ...
        *,
        ...
        # Bookkeeping the ``fusion-mlx telemetry status`` subcommand can
    ...
    def snapshot(self) -> dict[str, int]:
        """Cheap counters for ``fusion-mlx telemetry status``.
